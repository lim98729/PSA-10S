...
namespace MeiLibrary
{
	...
	unsafe public class mpiControl
	{
        ...
        public void NODE_DIGITAL_SEG_IN(int nodeNumber, int segNumber, int bitNumber, out bool enable, out RetMessage retMessage)
        {
            if (dev.NotExistHW.ZMP) { enable = true; retMessage = RetMessage.OK; return; }
            if (!isActivate)
            {
            ...
            bool tempEnable;
            retMessage = (RetMessage)control.NodeSegmentDigitalIn(nodeNumber, segNumber, bitNumber, &tempEnable);
            enable = tempEnable;
        }
        public void NODE_DIGITAL_SEG_OUT(int nodeNumber, int segNumber, int bitNumber, bool enable, out RetMessage retMessage)
        {
            if (dev.NotExistHW.ZMP) { retMessage = RetMessage.OK; return; }
            if (!isActivate)
            {
            ...
            retMessage = (RetMessage)control.NodeSegmentDigitalOut(nodeNumber, segNumber, bitNumber, enable);
        }
        public void NODE_DIGITAL_SEG_OUT(int nodeNumber, int segNumber, int bitNumber, out bool enable, out RetMessage retMessage)
        {
            if (dev.NotExistHW.ZMP) { enable = true; retMessage = RetMessage.OK; return; }
            if (!isActivate)
            {
            ...
            bool tempEnable;
            retMessage = (RetMessage)control.NodeSegmentDigitalOut(nodeNumber, segNumber, bitNumber, &tempEnable);
            enable = tempEnable;
        }
		#endregion
		#region userBuffer
	...
	unsafe public class mpiMotion
	{
		...
		//    if (!isActivate)
		//    {
		//        if (dev.NotExistHW.ZMP) retMessage = RetMessage.OK; else retMessage = RetMessage.INVALID;
		//        return;
		//    }
		//    axisMotionProfile pf;
        ...
        //For Multi
        public void moveCompare(double position, mpiMotion[] compareAxis, double comparePosition, bool LOGIC, bool ACTUAL, out RetMessage retMessage)
        {
            if (dev.NotExistHW.ZMP) { motionPos = position; retMessage = RetMessage.OK; return; }
            if (!isActivate)
            {
            ...
            //배열의 mpiMotion 중에서 가장 낮은 곳의 Position을 검색
            int lowerAxisNum = 0;
            double[] dPos = new double[compareAxis.Length];
            for (int i = 0; i < compareAxis.Length; i++)
            {
                compareAxis[i].actualPosition(out dPos[i], out retMessage);
                if (i != 0) { if (dPos[i] > dPos[i - 1]) lowerAxisNum = i; }
            }
            ...
            pf.speed.deceleration = g_to_c(config.speed.deceleration * config.speed.rate);
            pf.speed.jerkPercent = config.speed.jerkPercent;
            double comparePos = um_to_c(comparePosition, compareAxis[lowerAxisNum].config.gearA, compareAxis[lowerAxisNum].config.gearB);
            retMessage = (RetMessage)motion.moveCompare(pf.position, pf.speed.velocity, pf.speed.acceleration, pf.speed.deceleration, pf.speed.jerkPercent, 0, compareAxis[lowerAxisNum].config.axisNumber, comparePos, LOGIC, ACTUAL);
        }

        public void moveCompare(double position, axisMotionSpeed speed, mpiMotion[] compareAxis, double comparePosition, bool LOGIC, bool ACTUAL, out RetMessage retMessage)
        {
            if (dev.NotExistHW.ZMP) { motionPos = position; retMessage = RetMessage.OK; return; }
            if (!isActivate)
            {
		...
		public void clearPosition(out RetMessage retMessage)
		{
			...
			double position;
			actualPosition(out position, out retMessage); if (retMessage != RetMessage.OK) return;
	...
	public struct axisConfig
	{
		...
		public bool write(string savepath = "C:\\PROTEC\\Data")
		{
			...
			try
			{
                ...
                if (headNum != 0) fileName = filePath + unitCode.ToString() + axisCode.ToString() + headNum.ToString();
                else fileName = filePath + unitCode.ToString() + axisCode.ToString();
		...
		public bool read(string readpath = "C:\\PROTEC\\Data")
		 {
			...
			try
			{
                ...
                //homing.gpHomeSensor.nodeNumber = saveTuple[i]; i += 2;
    ...
    public struct mpiNodeDigtalOut
    {
        public int controlNumber;
        public int nodeNumber;
        public int segNumber;
