...
namespace DefineLibrary
{
	...
	public class CONTROL
	{
        ...
        public QueryTimer dwell2 = new QueryTimer();
	...
	public class EVENT
	{
        ...
        public delegate void InsertHandler_bool2(bool b, bool b2);
        ...
        public delegate void InsertHandler_addLoadcellData2(int seriesNum, double xVal, double yVal);
        ...
        public delegate void InsertHandler_refreshMGSlot(int slotCount);
        ...
        public static event InsertHandler_refreshMGSlot onAdd_refreshMGSlot;
        public static void refreshMGSlot(int slotCount)
        {
            if (onAdd_refreshMGSlot != null)
            {
                onAdd_refreshMGSlot(slotCount);
            }
        }
		#endregion
        ...
        public static event InsertHandler_int2 onAdd_refreshEditMagazine;
        public static void refreshEditMagazine(int mgNumber, int slotNumber)
        {
            if (onAdd_refreshEditMagazine != null)
            {
                onAdd_refreshEditMagazine(mgNumber, slotNumber);
            }
        }

        public static event InsertHandler_bool2 onAdd_refreshShowMagazine;
        public static void refreshShowMagazine(bool remove, bool show)
        {
            if (onAdd_refreshShowMagazine != null)
            {
                onAdd_refreshShowMagazine(remove, show);
            }
        }
        ...
        public static event InsertHandler_addLoadcellData2 onAdd_addLoadcellData2;
        public static void addLoadcellData2(int seriesNum, double xVal, double yVal)
        {
            if (onAdd_addLoadcellData2 != null)
            {
                onAdd_addLoadcellData2(seriesNum, xVal, yVal);
            }
        }
	...
	public class SQC
	{
        ...
        public const int CHECK_ATTACH_TILT = 2200;      // jhlim : Attach 후 Tilt 검사하기 위함

		public const int F_2M = 2300;				// force to Moving
		public const int F_M2PICK = 2350;			// pick down
		public const int F_PICK2M = 2400;			// pick up
		public const int F_M2PLACE = 2450;			// place down
		public const int F_PLACE2M = 2500;			// place up
		public const int F_M2PICKJIG = 2550;		// pick down jig
		public const int F_PICKJIG2M = 2600;		// pick up jig
		public const int F_M2PLACEJIG = 2650;		// place down jig
		public const int F_PLACEJIG2M = 2700;		// place up jig
		public const int F_M2PLACEREV = 2750;			// place down
        ...
        public const int FIND_PROJECTION_QUARTER_1 = 3600;
        public const int FIND_PROJECTION_QUARTER_2 = 3650;
        public const int FIND_PROJECTION_QUARTER_3 = 3700;
        public const int FIND_PROJECTION_QUARTER_4 = 3750;
	...
	public class QueryTimer
	{
		...
		long _lCtr2 = 0;
		long _lFreq = 0;
        static long st, ed, freq;
        static double process_check_time;  //usec
        static double process_time;  //usec
        static long process_cNum, process_aNum, process_fNum;
        ...
        public void process_check_start()
        {
            QueryPerformanceCounter(ref st);
        }
        public void process_check_end(long controlNumber, long axisNumber, long functionNumber)
        {
            QueryPerformanceCounter(ref ed);
            QueryPerformanceFrequency(ref freq);
            process_check_time = (ed - st) * 1000000.0 / freq;
            process_check_time = (int)process_check_time;

            if (process_check_time > process_time)
            {
                process_time = process_check_time;
                process_cNum = controlNumber;
                process_aNum = axisNumber;
                process_fNum = functionNumber;
            }
        }
	}
	#endregion
	...
	public enum DIAG_SEL_MODE
	{
        ...
        RetryAbortCancel,				// 검사 중 Retry, Abort, Skip 를 위함
        SkipAbortCancel,
        RetryNGCancel,
        RetryNGGoodCancel,
        IgnoreCancel,
	...
	public enum REQMODE
	{
        ...
        FIND_PROJECTION_QUARTER_1,
        FIND_PROJECTION_QUARTER_2,
        FIND_PROJECTION_QUARTER_3,
        FIND_PROJECTION_QUARTER_4,
	}
    ...
    public enum UnitCodeBoatMGSelect
    {
        INVALID = -1,
        MG1,
        MG2,
    ...
    public enum UnitCodeAxisNumber
    {
        ...
        HD_X = 2,
        [Description("Gantry Head Z1")]
        HD_Z1 = 3,
        [Description("Gantry Head T1")]
        HD_T1 = 4,
        [Description("Gantry Head Z2")]
        HD_Z2 = 5,
        [Description("Gantry Head T2")]
        HD_T2 = 6,
        ...
        CV_W = 10,
        [Description("Pedestal Width")]
        PD_W = 11,
        [Description("Stack Feeder Z1")]
        SF_Z1 = 12,
        [Description("Stack Feeder Z2")]
        SF_Z2 = 13,
        [Description("Pusher X")]
        PS_X = 14,
        [Description("Elevator Z")]
        ELEV_Z = 15,
    }

    public enum UnitCodeSlotSelect
    {
        INVALID = -1,
        START,
        END,
	...
	public enum MP_SF_Z
	{
		...
		DOWN = 20000,
		DOWN_4SLOT = 5000,
		MATERIAL_BOTTOM = 52500,  //자재하단
        MATERIAL_BOTTOM_4SLOT = 23000, //자재하단_4SLOT
		STROKE = 288000,
		STROKE_4SLOT = 280000,
	}
    ...
    public enum MP_MG_Z
    {
        ...
        MG1_READY = HOME_SENSOR + 220000,
        MG1_END = MG1_READY - 104000,
        MG2_READY = MG1_READY + 150000,
        MG2_END = MG2_READY - 104000,
        MG3_READY = MG2_READY + 150000,
        MG3_END = MG3_READY - 104000,
        READY = MG3_READY,
        STROKE = 1000000,
	...
	public class classBoard
	{
		...
		void writeSaveTuple(BOARD_ZONE boardZone, out bool b)
		{
			...
			try
			{
				...
				writeTuple("tmsInfo.pre_barcode", tmp.tmsInfo.pre_barcode, tmp.padCount, i, out i);
				...
				writeTuple("tmsInfo.post_barcode", tmp.tmsInfo.post_barcode, tmp.padCount, i, out i);
				...
				writeTuple("pad.placeTime", tmp.pad.placeTime, tmp.padCount, i, out i);
				...
				writeTuple("pad.status", tmp.pad.status, tmp.padCount, i, out i);
				...
				writeTuple("pad.ulc.score", tmp.pad.ulc.score, tmp.padCount, i, out i);
				...
				writeTuple("pad.hdc_pre_p1.score", tmp.pad.hdc_pre_p1.score, tmp.padCount, i, out i);
				...
				writeTuple("pad.hdc_pre_p1.x", tmp.pad.hdc_pre_p1.x, tmp.padCount, i, out i);
				...
				writeTuple("pad.hdc_pre_p1.y", tmp.pad.hdc_pre_p1.y, tmp.padCount, i, out i);
				...
				writeTuple("pad.hdc_pre_p1.angle", tmp.pad.hdc_pre_p1.angle, tmp.padCount, i, out i);
				...
				writeTuple("pad.hdc_pre_p2.score", tmp.pad.hdc_pre_p2.score, tmp.padCount, i, out i);
				...
				writeTuple("pad.hdc_pre_p2.x", tmp.pad.hdc_pre_p2.x, tmp.padCount, i, out i);
				...
				writeTuple("pad.hdc_pre_p2.y", tmp.pad.hdc_pre_p2.y, tmp.padCount, i, out i);
				...
				writeTuple("pad.hdc_pre_p2.angle", tmp.pad.hdc_pre_p2.angle, tmp.padCount, i, out i);
				...
				writeTuple("pad.hdc_pre.score", tmp.pad.hdc_pre.score, tmp.padCount, i, out i);
				...
				writeTuple("pad.hdc_pre.x", tmp.pad.hdc_pre.x, tmp.padCount, i, out i);
				...
				writeTuple("pad.hdc_pre.y", tmp.pad.hdc_pre.y, tmp.padCount, i, out i);
				...
				writeTuple("pad.hdc_pre.angle", tmp.pad.hdc_pre.angle, tmp.padCount, i, out i);
				...
				writeTuple("pad.hdc_post.score", tmp.pad.hdc_post.score, tmp.padCount, i, out i);
				...
				writeTuple("pad.hdc_post.x", tmp.pad.hdc_post.x, tmp.padCount, i, out i);
				...
				writeTuple("pad.hdc_post.y", tmp.pad.hdc_post.y, tmp.padCount, i, out i);
				...
				writeTuple("pad.hdc_post.angle", tmp.pad.hdc_post.angle, tmp.padCount, i, out i);
				...
				//20130618. kimsong.
				writeTuple("pad.pd_laser_offset.laser_c1", tmp.pad.pd_laser_offset.laser_c1, tmp.padCount, i, out i);
				...
				writeTuple("pad.pd_laser_offset.laser_c2", tmp.pad.pd_laser_offset.laser_c2, tmp.padCount, i, out i);
				...
				writeTuple("pad.pd_laser_offset.laser_c3", tmp.pad.pd_laser_offset.laser_c3, tmp.padCount, i, out i);
				...
				writeTuple("pad.pd_laser_offset.laser_c4", tmp.pad.pd_laser_offset.laser_c4, tmp.padCount, i, out i);
				...
				writeTuple("pad.pd_laser_offset.laser_result", tmp.pad.pd_laser_offset.laser_result, tmp.padCount, i, out i);
				...
				//20130630. kimsong.
				writeTuple("pad.mapResult", tmp.pad.mapResult, tmp.padCount, i, out i);
...
// 
				...
				if (status == PAD_STATUS.ATTACH_OVERPRESS.ToString()) return PAD_STATUS.ATTACH_OVERPRESS;
				if (status == PAD_STATUS.PEDESTAL_SUC_ERR.ToString()) return PAD_STATUS.PEDESTAL_SUC_ERR;
                if (status == PAD_STATUS.TILT_OK.ToString()) return PAD_STATUS.TILT_OK;
		...
		public void padStatusOrg(BOARD_ZONE boardZone, int index, PAD_STATUS status, out bool b)
		{
			try
			{
				...
				{ 
					...
					if (status == PAD_STATUS.SKIP) working.tmsInfo.mapInfo[index] = (int)TMSCODE.SKIP;							// Edit에서 Ready상태로 변경했으니 다시 찍어야 한다.
                    else if (status == PAD_STATUS.READY) working.tmsInfo.mapInfo[index] = (int)TMSCODE.READY;							// Edit에서 Ready상태로 변경했으니 다시 찍어야 한다.
// 					else if (status == PAD_STATUS.PRE_VISION_ERROR) working.tmsInfo.mapInfo[index] = (char)TMSCODE.PCB_ERROR;		// 단지 호환성을 위해 남겨둔 Factor
				...
				{ 
                    ...
                    if (status == PAD_STATUS.SKIP) workingedit.tmsInfo.mapInfo[index] = (int)TMSCODE.SKIP;							// Edit에서 Ready상태로 변경했으니 다시 찍어야 한다.
                    else if (status == PAD_STATUS.READY) workingedit.tmsInfo.mapInfo[index] = (int)TMSCODE.READY;							// Edit에서 Ready상태로 변경했으니 다시 찍어야 한다.
                    else if (status == PAD_STATUS.INVALID) workingedit.tmsInfo.mapInfo[index] = (int)TMSCODE.SKIP;
...
// 					else if (tmp.pad.status[i] == PAD_STATUS.PRE_VISION_ERROR.ToString()) skip++;
        ...
        public void padIndex(out int ix, out int iy, PAD_STATUS status, out bool b)
        {
            try
            {
                int i = working.pad.status.TupleFind(status.ToString());
                if (i == 0)
                {
                    ix = 0; iy = 0;
                }
        ...
        public void padIndex(out int ix, out int iy, out bool b, out int remainCount)
        {
            try
            {
                remainCount = 0;
                ...
                for (int j = 0; j < padCountX * padCountY; j++)
                {
                    if (working.pad.status.SArr[j] == PAD_STATUS.READY.ToString())
                        remainCount++;
                }
            }
            catch
        ...
        //    try
        //    {
        //        for (int index = 0; index < padCountX * padCountY; index++)
        //        {
        //            if (working.pad.status.SArr[index] == PAD_STATUS.READY.ToString())
        //            {
        //                if (index == 0)
        //                {
        //                    ix = 0; iy = 0;
        ...
        //                if (WorkAreaControl.workArea[ix, iy] == 0) continue;
...
// 		eSV_LAST = 74
	...
	public enum ALARM_CODE		// MPC에 1051부터 1400까지 350개가 할당되어 있다.
	{
        ...
        E_SYSTEM_SW_MAGAZINE_LIST_NONE = 14,
        E_SYSTEM_SW_PUSHER_LIST_NONE = 15, 
        ...
        E_SYSTEM_SW_MAGAZINE_NOT_READY = 28,
        E_SYSTEM_SW_PUSHER_NOT_READY = 29,
        ...
        E_PUSHER_IO_JAM_DETECT = 190,
        E_PUSHER_IO_BOAT_IN_SENSOR_DETECT = 191,

        E_MAGAZINE_IO_BOAT_SENSOR_DETECT = 195,
        E_MAGAZINE_IO_AREA_SENSOR_DETECT = 196,
        E_MAGAGINE_NOT_EXIST = 197,
        E_MAGAGINE_STATUS_FULL = 198,
    ...
    public enum UnitCodeAnalogInput
    {
        INVAILD = -1,
        VAC_HEAD1 = 1,
        VAC_HEAD2 = 3,
	...
	public class UtilityControl
	{
		...
		public static void readColorConfig()
		{
            ...
            sTemp = utilconfig.GetString("TiltOK", "ffffffff");
            colorCode[(int)COLORCODE.TILT_OK] = HexToColor(sTemp);
		}
		...
		public static void writeColorConfig()
		{
            ...
            utilconfig.WriteString("TiltOK", colorCode[(int)COLORCODE.TILT_OK].Name);
		}
