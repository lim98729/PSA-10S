...
namespace HalconLibrary
{
	...
	public class halcon
	{
        ...
        public halcon_projection[] projectionEdge = new halcon_projection[4];
		...
		public void activate(HTuple cameraNumber, HTuple name, out string returnStr)
		{
			try
			{
                ...
                for (int i = 0; i < 4; i++)
                {
                    HOperatorSet.GenEmptyObj(out projectionEdge[i].Region);
                    HOperatorSet.GenEmptyObj(out projectionEdge[i].ImageReduced);

                    projectionEdge[i].setDefault(i);
                    projectionEdge[i].read(i);
                }
        ...
        public bool createCornerRegion(HTuple number)
        {
            try
            {
                if (!isActivate) return false;
                ...
                //HOperatorSet.DrawRectangle1(window.handle, out projectionEdge[number].createRow1, out projectionEdge[number].createColumn1, out projectionEdge[number].createRow2, out projectionEdge[number].createColumn2);
                projectionEdge[number].createRow1 = height / 5;
                projectionEdge[number].createRow2 = height - height / 5;
                projectionEdge[number].createColumn1 = width / 5;
                projectionEdge[number].createColumn2 = width - width / 5;
        ...
        public bool refreshProjectionEdge()
        {
            try
            {
                ...
                if (projectionEdge[refresh_reqModelNumber].isCreate == "true")
                {
                    HOperatorSet.SetColor(window.handle, "pink");
                    HOperatorSet.DispCross(window.handle, acq.height / 2, acq.width / 2, acq.height * 0.8, 0);

                    //DispBinaryRectImage(refresh_reqModelNumber);

                    HOperatorSet.SetLineWidth(window.handle, 2);
                    HOperatorSet.SetColor(window.handle, "green");
                    HOperatorSet.DispCross(window.handle, projectionEdge[refresh_reqModelNumber].tempY, projectionEdge[refresh_reqModelNumber].tempX, 100, 0);
                    HOperatorSet.SetLineWidth(window.handle, 1);
                }
                else
                {
                ...
                string str;
                str = "Projection Edge" + "\n";
                str += "X         : " + Math.Round((double)projectionEdge[refresh_reqModelNumber].resultX, 2).ToString() + "\n";
                str += "Y         : " + Math.Round((double)projectionEdge[refresh_reqModelNumber].resultY, 2).ToString() + "\n";
                str += "Angle  : " + Math.Round((double)projectionEdge[refresh_reqModelNumber].resultAngle, 2).ToString() + "\n";
                str += "Angle2  : " + Math.Round((double)projectionEdge[refresh_reqModelNumber].resultAngle2, 2).ToString() + "\n";
                str += "Time      : " + Math.Round((double)projectionEdge[refresh_reqModelNumber].resultTime, 2).ToString() + "\n";
                messageResult(str);
                #endregion
                refresh_req = false;
                return true;
        ...
        public void DispBinaryRectImage(HTuple number)
        {
            try
            {
                model[number].Region.Dispose();
                HOperatorSet.GenRectangle1(out projectionEdge[number].Region,
                    projectionEdge[number].createRow1, projectionEdge[number].createColumn1,
                    projectionEdge[number].createRow2, projectionEdge[number].createColumn2);
                HOperatorSet.ReduceDomain(acq.Image, projectionEdge[number].Region, out projectionEdge[number].ImageReduced);
                ...
                HOperatorSet.Threshold(projectionEdge[number].ImageReduced, out hObj, projectionEdge[number].thresholdMin, projectionEdge[number].thresholdMax);
                HOperatorSet.FillUp(hObj, out hObj);
                HOperatorSet.OpeningCircle(hObj, out hObj, 3.5);
                HOperatorSet.Connection(hObj, out hObj);
                ...
                cornerRgn.left = projectionEdge[number].createColumn1;
                cornerRgn.right = projectionEdge[number].createColumn2;
                cornerRgn.top = projectionEdge[number].createRow1;
                cornerRgn.bottom = projectionEdge[number].createRow2;

                DPOINT cornerPoint;
                double angle = -1;
                double angle2 = -1;
                projectionCenter(acq.Image, acq.width, acq.height, cornerRgn, projectionEdge[number].edgeFilter,
                    projectionEdge[number].type, projectionEdge[number].direction, projectionEdge[number].quardrant, blobCenter, out cornerPoint, out angle, out angle2);

                projectionEdge[number].tempX = cornerPoint.x;
                projectionEdge[number].tempY = cornerPoint.y;
                projectionEdge[number].resultAngle = angle;
                projectionEdge[number].resultAngle2 = angle2;
                projectionEdge[refresh_reqModelNumber].resultTime = dwell.Elapsed;
            }
            catch (HalconException ex)
            {
                projectionEdge[number].resultX = -1;
                projectionEdge[number].resultY = -1;
                projectionEdge[number].resultAngle = -1;
                projectionEdge[number].resultAngle2 = -1;
            }
        }

        public void DispBinaryRectImage(HTuple number, int quardrant, int type, int dir, int edge, int minTh, int maxTh)
        {
            projectionEdge[number].resultX = -1;
            projectionEdge[number].resultY = -1;
            projectionEdge[number].resultAngle = -1;

            projectionEdge[number].tempX = -1;
            projectionEdge[number].tempY = -1;
            ...
            projectionEdge[number].quardrant = quardrant;
            projectionEdge[number].type = type;
            projectionEdge[number].direction = dir;
            projectionEdge[number].edgeFilter = edge;
            projectionEdge[number].thresholdMin = minTh;
            projectionEdge[number].thresholdMax = maxTh;
            ...
            projectionEdge[number].tempX = cornerPoint.x;
            projectionEdge[number].tempY = cornerPoint.y;
            projectionEdge[number].resultX = ((-acq.width / 2) + cornerPoint.x) * acq.ResolutionX;
            projectionEdge[number].resultY = ((-acq.height / 2) + cornerPoint.y) * acq.ResolutionY;
            projectionEdge[number].resultAngle = angle;
            projectionEdge[number].resultAngle2 = angle2;

            projectionEdge[number].write(number);
        }
            ...
            try 
            {
                ...
                if (nEdgeDir == 0)       // in -> out
                {
                    ...
                    case (int)QUARDRANT.QUARDRANT_LT:
			            ...
			            nLength1 =  Convert.ToInt32((blobCenter.x - rect.left)/2);
			            nLength2 =  Convert.ToInt32((blobCenter.y - rect.top )/2);
                    ...
                    case (int)QUARDRANT.QUARDRANT_RT:
			            ...
			            nLength1 = Convert.ToInt32((rect.right - blobCenter.x)/2);
			            nLength2 = Convert.ToInt32((blobCenter.y - rect.top  )/2);
                    ...
                    case (int)QUARDRANT.QUARDRANT_LB:
			            ...
			            nLength1 = Convert.ToInt32((blobCenter.x - rect.left  )/2);
			            nLength2 = Convert.ToInt32((rect.bottom - blobCenter.y)/2);
		...
		public void findCircleCenter(out RetMessage retMessage, out string errorMessage)
		{
			try
			{
				...
				#region 에지 추출
                // 160524. jhlim : 임시
                HOperatorSet.Threshold(circleCenter.ImageReduced, out circleCenter.ImageReduced, 0, 200);
				circleCenter.Edges.Dispose();
				HOperatorSet.GrayClosingRect(circleCenter.ImageReduced, out circleCenter.ImageReduced, 10, 10);
				HOperatorSet.ErosionRectangle1(circleCenter.Region, out circleCenter.Region, 3, 3);
				HOperatorSet.ReduceDomain(circleCenter.ImageReduced, circleCenter.Region, out circleCenter.ImageReduced);
...
// 				for (int i = 0; i < model[refresh_reqModelNumber].resultScore.Length; i++) str += Math.Round((double)model[refresh_reqModelNumber].resultScore.TupleSelect(i), 2).ToString() + " "; str += "\n";
        ...
        public void drawCross(out int rowPos, out int colPos)
        {
            HTuple btn = new HTuple();
            HTuple row = new HTuple();
            HTuple col = new HTuple();
            HOperatorSet.GetMpositionSubPix(window.handle, out row, out col, out btn);
            rowPos = row.I;
            colPos = col.I;
        }
        ...
        public bool writeLogDumpImage(HTuple fileName)
        {
            try
            {
                if (dev.NotExistHW.CAMERA) return false;
                ...
                if (!Directory.Exists(filePath)) Directory.CreateDirectory(filePath);
                HTuple saveFileName = filePath + DateTime.Now.Hour.ToString("d2") + DateTime.Now.Minute.ToString("d2") + DateTime.Now.Second.ToString("d2") + "_" + fileName;
                HOperatorSet.DumpWindowImage(out dumpImage, window.handle);
                //HOperatorSet.WriteImage(acq.Image, "jpeg", 0, saveFileName);
                HOperatorSet.WriteImage(dumpImage, "tiff", 0, saveFileName);
                return true;
            }
            catch (HalconException ex)
            {
		...
		public bool paraApply()
		{
			...
			try
			{
				...
				if (grabber.name == "GigEVision")
				{
					...
					HOperatorSet.SetFramegrabberParam(handle, "ReverseX", ReverseX); i++;
					if (DeviceModelName == "acA2040-25gm") HOperatorSet.SetFramegrabberParam(handle, "ReverseY", ReverseY); i++;
                    //if (DeviceModelName == "acA1300-60gm") HOperatorSet.SetFramegrabberParam(handle, "ReverseY", 1); i++;
					HOperatorSet.SetFramegrabberParam(handle, "GainAuto", GainAuto); i++;
                    if (GainAuto == "Off") HOperatorSet.SetFramegrabberParam(handle, "GainRaw", 3); i++;
    ...
    public struct halcon_projection
    {
        ...
        public bool write(HTuple cameraNumber)
        {
            try
            {
                ...
                if (!Directory.Exists(filePath)) Directory.CreateDirectory(filePath);
                fileName = filePath + "projection" + cameraNumber.ToString();
                writeSaveTuple();
                HOperatorSet.WriteTuple(saveTuple, fileName + ".tup");
                isCreate = "true";
                return true;
	...
	public class halcon_edgeIntersection
	{
		...
		public void create(HObject image, halcon_window window, HTuple resolutionX, HTuple resolutionY, QUARTER_NUMBER quarter, out bool b)
		{
			try
			{
				...
				else if (quarterNumber == QUARTER_NUMBER.FIRST)
				{
					createVColumn1 = ox - shortLength * cropArea;
					createVColumn2 = ox + shortLength;
