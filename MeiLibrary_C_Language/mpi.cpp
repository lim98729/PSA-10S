...
public:
	...
	MPI_RESULT NodeSegmentDigitalOut(long nodeNumber, long segment, long bitNumber, bool cmd)
	{
		if(!isActivate) return -1;
		uint32_t state;
		if(cmd) state = 1; else state = 0;
		mpiSqNodeSegmentDigitalOutSet(sqNode[controlNumber][nodeNumber], segment , bitNumber, 1, state, 0);
	}
	MPI_RESULT NodeSegmentDigitalOut(long nodeNumber, long segment, long bitNumber, bool &cmd)
	{
		if(!isActivate) return -1;
		uint32_t state;
		returnValue = mpiSqNodeSegmentDigitalOutGet(sqNode[controlNumber][nodeNumber], segment, bitNumber, 1, &state);
		if(state != 0) cmd = true; else cmd = false;
		return returnValue;
	}
	MPI_RESULT NodeSegmentDigitalIn(long nodeNumber, long segment, long bitNumber, bool &enable)
	{
		if(!isActivate) return -1;
		uint32_t state;
		returnValue = mpiSqNodeSegmentDigitalIn(sqNode[controlNumber][nodeNumber], segment, bitNumber, 1, &state);
		if(state != 0) enable = true; else enable = false;
		return returnValue;
	}
...
public:
	...
	MPI_RESULT capturePosition(double &latchedValue)
	{
		...
		if (config.feedback[0].ratio.numerator < 0)
		{
			returnValue = mpiCaptureStatus(capture[controlNumber][axisNumber], &captureStatus);
			latchedValue -= captureStatus.latchedValue;
			return returnValue;
