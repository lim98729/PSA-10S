	...
	public partial class BottomRight : UserControl
	{
		...
		void bottomRightPanelMode(BOTTOM_RIGHT_PANEL mode)
		{
			...
			else
			{
                ...
                BottomRight_Pusher.Visible = false; BottomRight_Pusher.timer.Enabled = false;
                BottomRight_Magazine.Visible = false; BottomRight_Magazine.timer.Enabled = false;
                ...
                if (mode == BOTTOM_RIGHT_PANEL.PUSHER)
                {
                    mc.user.selectedIOMenu = BOTTOM_RIGHT_PANEL.PUSHER.ToString();

                    BottomRight_Pusher.Dock = DockStyle.Fill;
                    BottomRight_Pusher.Visible = true;
                    BottomRight_Pusher.timer.Enabled = true;
                    str = BT_PUSHER.Text;
                }

                if (mode == BOTTOM_RIGHT_PANEL.MAGAZINE)
                {
                    mc.user.selectedIOMenu = BOTTOM_RIGHT_PANEL.MAGAZINE.ToString();

                    BottomRight_Magazine.Dock = DockStyle.Fill;
                    BottomRight_Magazine.Visible = true;
                    BottomRight_Magazine.timer.Enabled = true;
                    str = BT_MAGAZINE.Text;
                }
		...
		private void Control_Click(object sender, EventArgs e)
		{
            ...
            if (sender.Equals(BT_PUSHER)) bottomRightPanelMode(BOTTOM_RIGHT_PANEL.PUSHER);
            if (sender.Equals(BT_MAGAZINE)) bottomRightPanelMode(BOTTOM_RIGHT_PANEL.MAGAZINE);
        ...
        private void BottomRight_Load(object sender, EventArgs e)
        {
            if (mc.para.ETC.unloaderControl.value == 1)
            {
                BT_MAGAZINE.Visible = true;
