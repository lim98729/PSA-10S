...
namespace PSA_Application
{
	public partial class FormJogPad : Form
	{
		...
		private void FormJogPad_Load(object sender, EventArgs e)
		{
			...
			{
                tempCon.x.value = mc.para.CAL.Tool_Con[selectedTool].x.value;
                tempCon.y.value = mc.para.CAL.Tool_Con[selectedTool].y.value;
			...
			if (jogMode == JOGMODE.HDC_PD_P1 || jogMode == JOGMODE.HDC_PD_P2)
			{
                BT_AutoCalibration.Visible = false;
		...
		void control()
		{
			...
			while (true)
			{
				...
				{
					#region moving
                    mc.para.CAL.HDC_TOOL[selectedTool].x.value = dataX.value;
                    mc.para.CAL.HDC_TOOL[selectedTool].y.value = dataY.value;
                    posX = mc.hd.tool.tPos.x[selectedTool].ULC;
                    posY = mc.hd.tool.tPos.y[selectedTool].ULC;
				...
				if (jogMode == JOGMODE.HDC_TOUCHPROBE)
				{
                    ...
                    //posX += mc.coor.MP.HD.X.TOUCHPROBE.value + dataX.value;
                    //posY = mc.coor.MP.TOOL.Y.CAMERA.value + mc.para.CAL.machineRef[(int)UnitCodeMachineRef.REF0].y.value;
                    //posY += mc.coor.MP.HD.Y.TOUCHPROBE.value + dataY.value;
				...
				if (jogMode == JOGMODE.HDC_LOADCELL)
				{
					#region moving
                    //posX = mc.coor.MP.TOOL.X.CAMERA.value + mc.para.CAL.machineRef[(int)UnitCodeMachineRef.REF0].x.value;
                    //posX += mc.coor.MP.HD.X.LOADCELL.value + dataX.value;
                    //posY = mc.coor.MP.TOOL.Y.CAMERA.value + mc.para.CAL.machineRef[(int)UnitCodeMachineRef.REF0].y.value;
                    //posY += mc.coor.MP.HD.Y.LOADCELL.value + dataY.value;
				...
				if (jogMode == JOGMODE.HDC_PD_P1)
				{
                    ...
                    //posX += dataX.value;
                    //posY += dataY.value;
                    //mc.pd.jogMove(posX, posY, out ret.message, true, false); if (ret.message != RetMessage.OK) { mc.message.alarmMotion(ret.message); goto EXIT; }
                    //#endregion
                    ////mc.idle(100);
                    //mc.hdc.LIVE = false;
                    //mc.hdc.rectangleFind();
				...
				{
                    //#region moving
                    //posX = mc.coor.MP.PD.W.READY.value;
                    //posX += dataX.value;
                    //mc.pd.jogMove(posX, out ret.message, true, false); if (ret.message != RetMessage.OK) { mc.message.alarmMotion(ret.message); goto EXIT; }
                    //#endregion
                    //mc.hdc.LIVE = false;
                    //mc.hdc.rectangleFind();
                    //mc.hdc.LIVE = true; mc.hdc.liveMode = REFRESH_REQMODE.RECTANGLE_CENTER;
				...
				if (jogMode == JOGMODE.HDC_BD_EDGE)
				{
					#region moving
                    //posX = mc.coor.MP.TOOL.X.CAMERA.value + mc.para.CAL.machineRef[(int)UnitCodeMachineRef.REF0].x.value;
					//posX += (double)MP_HD_X.BD_EDGE + dataX.value; // LJS 없는변수
                    //posY = mc.coor.MP.TOOL.Y.CAMERA.value + mc.para.CAL.machineRef[(int)UnitCodeMachineRef.REF0].y.value;
                ...
                {
                    #region moving
                    posX = mc.para.UD.PusherPos.value = dataX.value;
                    mc.ps.jogMove(posX, out ret.message); if (ret.message != RetMessage.OK) { mc.message.alarmMotion(ret.message); goto EXIT; }
                    #endregion
                }
                #endregion
		...
		private void Control_Click(object sender, EventArgs e)
		{
			...
			if (sender.Equals(BT_ESC) || sender.Equals(BT_Set))
			{
				...
				if (jogMode == JOGMODE.ULC_CALIBRATION ||
                    ...
                    jogMode == JOGMODE.TOOL_ANGLEOFFSET)
				{
					mc.ulc.LIVE = false;
				}
				if (jogMode == JOGMODE.ULC)
                ...
                if (sender.Equals(BT_Set))
                {
                    if (jogMode == JOGMODE.ULC_TOOL)
                    {
                        mc.para.CAL.Tool_Con[selectedTool].x.value = tempCon.x.value;
                        mc.para.CAL.Tool_Con[selectedTool].y.value = tempCon.y.value;
                    }
                }
				this.Close();
			}
			...
			if (sender.Equals(BT_AutoCalibration))
			{
				...
				if (jogMode == JOGMODE.ULC_TOOL)
				{
                    ...
                    posX = mc.hd.tool.tPos.x[selectedTool].ULC;
                    posY = mc.hd.tool.tPos.y[selectedTool].ULC;
                    posT = mc.para.CAL.toolAngleOffset[selectedTool].value;
                    mc.hd.tool.jogMoveXYT(selectedTool, posX, posY, posT, out ret.message); if (ret.message != RetMessage.OK) { mc.message.alarmMotion(ret.message); goto EXIT; }
                    mc.ulc.circleFind();
                    if ((double)mc.ulc.cam.circleCenter.resultRadius != -1)
                    {
                        ...
                        mc.para.CAL.HDC_TOOL[selectedTool].x.value = dataX.value;
                        mc.para.CAL.HDC_TOOL[selectedTool].y.value = dataY.value;
                        posX = mc.hd.tool.tPos.x[selectedTool].ULC;// +ctX;
                        posY = mc.hd.tool.tPos.y[selectedTool].ULC;// +ctY;
                        posT = mc.para.CAL.toolAngleOffset[selectedTool].value;
                        mc.hd.tool.jogMoveXYT(selectedTool, posX, posY, posT, out ret.message); if (ret.message != RetMessage.OK) { mc.message.alarmMotion(ret.message); goto EXIT; }
                        #endregion
                    }
                    ...
                    for (int i = 0; i < stepCnt; i++)
                    {
                        ...
                        posX = mc.hd.tool.tPos.x[selectedTool].ULC;
                        posY = mc.hd.tool.tPos.y[selectedTool].ULC;
                        posT = mc.para.CAL.toolAngleOffset[selectedTool].value + (stepDeg * i);
                        mc.hd.tool.jogMoveXYT(selectedTool, posX, posY, posT, out ret.message); if (ret.message != RetMessage.OK) { mc.message.alarmMotion(ret.message); goto EXIT; }
                        #endregion
                        mc.ulc.circleFind();
                        if ((double)mc.ulc.cam.circleCenter.resultRadius != -1)
                        {
                            ...
                            mc.para.CAL.HDC_TOOL[selectedTool].x.value = dataX.value;
                            mc.para.CAL.HDC_TOOL[selectedTool].y.value = dataY.value;
                            posX = mc.hd.tool.tPos.x[selectedTool].ULC + centerX[i];
                            posY = mc.hd.tool.tPos.y[selectedTool].ULC + centerY[i];
                            posT = mc.para.CAL.toolAngleOffset[selectedTool].value + (stepDeg * i);
                            mc.hd.tool.jogMoveXYT(selectedTool, posX, posY, posT, out ret.message); if (ret.message != RetMessage.OK) { mc.message.alarmMotion(ret.message); goto EXIT; }
                            #endregion
                        }
                    ...
                    for (int i = 0; i < stepCnt2; i++)
                    {
                        ...
                        posX = mc.hd.tool.tPos.x[selectedTool].ULC;
                        posY = mc.hd.tool.tPos.y[selectedTool].ULC;
                        posT = mc.para.CAL.toolAngleOffset[selectedTool].value + (stepDeg2 * i);
                        mc.hd.tool.jogMoveXYT(selectedTool, posX, posY, posT, out ret.message); if (ret.message != RetMessage.OK) { mc.message.alarmMotion(ret.message); goto EXIT; }
                        #endregion
                        mc.ulc.circleFind();
                        if ((double)mc.ulc.cam.circleCenter.resultRadius != -1)
                        {
                            ...
                            mc.para.CAL.HDC_TOOL[selectedTool].x.value = dataX.value;
                            mc.para.CAL.HDC_TOOL[selectedTool].y.value = dataY.value;
                            posX = mc.hd.tool.tPos.x[selectedTool].ULC + centerX2[i];
                            posY = mc.hd.tool.tPos.y[selectedTool].ULC + centerY2[i];
                            posT = mc.para.CAL.toolAngleOffset[selectedTool].value + (stepDeg2 * i);
                            mc.hd.tool.jogMoveXYT(selectedTool, posX, posY, posT, out ret.message); if (ret.message != RetMessage.OK) { mc.message.alarmMotion(ret.message); goto EXIT; }
                            #endregion
                        }
                    ...
                    tempCon.x.value = (centerX.Average() + centerX2.Average()) / 2;
                    tempCon.y.value = (centerY.Average() + centerY2.Average()) / 2;
                    mc.log.debug.write(mc.log.CODE.INFO, "회전중심 X : " + tempCon.x.value.ToString() + ", 회전중심 Y : " + tempCon.y.value.ToString());
                    #endregion
                ...
                //        posX = mc.pd.pos.x.P1;
                //        posY = mc.pd.pos.y.P1;
                //        //posX = mc.coor.MP.PD.X.P1_FR.value; 
				...
				RETRY:
					...
					#region moving
                    posX = mc.coor.MP.TOOL.X.CAMERA.value + mc.para.CAL.machineRef[(int)UnitCodeMachineRef.REF0].x.value;
                    posX += mc.coor.MP.HD.X.ULC.value + dataX.value;
                    posY = mc.coor.MP.TOOL.Y.CAMERA.value + mc.para.CAL.machineRef[(int)UnitCodeMachineRef.REF0].y.value;
                    posY += mc.coor.MP.HD.Y.ULC.value + dataY.value;
					mc.hd.tool.jogMove(posX, posY, out ret.message); if (ret.message != RetMessage.OK) { mc.message.alarmMotion(ret.message); goto EXIT; }
					#endregion
					mc.idle(100);
					if (retry++ < 3) goto RETRY;
				...
				if (jogMode == JOGMODE.HDC_CALIBRATION)
				{
					...
					posX = mc.hd.tool.cPos.x.REF0;
					posY = mc.hd.tool.cPos.y.REF0;
                    //posX = mc.coor.MP.TOOL.X.CAMERA.value + mc.coor.MP.HD.X.REF0.value + mc.para.CAL.machineRef[(int)UnitCodeMachineRef.REF0].x.value;
                    //posY = mc.coor.MP.TOOL.Y.CAMERA.value + mc.coor.MP.HD.Y.REF0.value + mc.para.CAL.machineRef[(int)UnitCodeMachineRef.REF0].y.value;
				...
				if (jogMode == JOGMODE.ULC_CALIBRATION)
				{
                    ...
                    #region moving
                    //posX = mc.coor.MP.TOOL.X.TOOL.value + mc.para.CAL.HDC_TOOL.x.value + mc.para.CAL.machineRef[(int)UnitCodeMachineRef.REF0].x.value;
                    //posX += mc.coor.MP.HD.X.ULC.value + +mc.para.CAL.ulc.x.value;
                    //posY = mc.coor.MP.TOOL.Y.TOOL.value + mc.para.CAL.HDC_TOOL.y.value + mc.para.CAL.machineRef[(int)UnitCodeMachineRef.REF0].y.value;
                    //posY += mc.coor.MP.HD.Y.ULC.value + +mc.para.CAL.ulc.y.value;
			...
			if (sender.Equals(BT_TEACH))
			{
                ...
                {
                    #region 기존 패턴 삭제
                    mc.para.ULC.modelVisionCAL.isCreate.value = (int)BOOL.FALSE;
                    mc.ulc.cam.model[(int)ULC_MODEL.VISION_RESOLUTION_NCC].delete();
                    #endregion
                    mc.ulc.cam.model[(int)ULC_MODEL.VISION_RESOLUTION_NCC].algorism = MODEL_ALGORISM.NCC.ToString();
                    mc.ulc.cam.createModel((int)ULC_MODEL.VISION_RESOLUTION_NCC);
                    mc.ulc.cam.createFind((int)ULC_MODEL.VISION_RESOLUTION_NCC);
                    mc.para.ULC.modelVisionCAL.isCreate.value = (int)BOOL.TRUE;
                    if (mc.ulc.cam.model[(int)ULC_MODEL.VISION_RESOLUTION_NCC].isCreate == "false")
                        mc.para.ULC.modelVisionCAL.isCreate.value = (int)BOOL.FALSE;
                }
			...
			if (sender.Equals(BT_TEST))
			{
                ...
                if (jogMode == JOGMODE.ULC_CALIBRATION || jogMode == JOGMODE.ULC_ANGLE_CALIBRATION)
                {
                    mc.ulc.cam.grabSofrwareTrigger();
                    mc.ulc.cam.writeGrabImage("CAL");
                    //mc.ulc.circleFind();
                    //#region ULC.req
                    //if (mc.para.ULC.modelVisionCAL.isCreate.value == (int)BOOL.TRUE)
                    //{
                    //    mc.ulc.reqMode = REQMODE.FIND_MODEL;
                    //    mc.ulc.reqModelNumber = (int)ULC_MODEL.VISION_RESOLUTION_NCC;
                    //}
                    //else
                    //{
                    ...
                    //if (mc.para.ULC.modelVisionCAL.isCreate.value == (int)BOOL.TRUE)
                    //{
                    //    rX = mc.ulc.cam.model[(int)ULC_MODEL.VISION_RESOLUTION_NCC].resultX;
                    //    rY = mc.ulc.cam.model[(int)ULC_MODEL.VISION_RESOLUTION_NCC].resultY;
                    //    rT = mc.ulc.cam.model[(int)ULC_MODEL.VISION_RESOLUTION_NCC].resultAngle;
                    //}
                    //#endregion
                }
				else if (jogMode == JOGMODE.PATTERN_TRAYREVERSE1)
		...
		private void timer_Tick(object sender, EventArgs e)
		{
			...
			{
				timer.Enabled = false;
				mc.hd.tool.actualPosition_AxisT(selectedTool, out pos, out ret.message);
		...
		private void BT_JogT_CCW_Click(object sender, EventArgs e)
		{
			...
			{
				#region moving
				mc.hd.tool.jogPlusMoveT(0, 180, out ret.message);
				#endregion
				mc.ulc.LIVE = false;
				mc.ulc.circleFind();
				mc.ulc.LIVE = true; mc.ulc.liveMode = REFRESH_REQMODE.CIRCLE_CENTER;
            ...
            #region FR_TOOL_ANGLE
            if (jogMode == JOGMODE.TOOL_ANGLEOFFSET)
            {
                #region moving
                mc.hd.tool.jogPlusMoveT(selectedTool, +dX, out ret.message);
                #endregion
                mc.ulc.LIVE = false;
                mc.ulc.circleFind();
                mc.ulc.LIVE = true; mc.ulc.liveMode = REFRESH_REQMODE.CIRCLE_CENTER;
