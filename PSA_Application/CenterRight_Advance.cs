	...
	public partial class CenterRight_Advance : UserControl
	{
		...
		void refresh()
		{
			...
			else
			{
                ...
                if (mc.para.ETC.unloaderControl.value == 0) { BT_UnloaderControl.Text = "OFF"; BT_UnloaderControl.Image = Properties.Resources.YellowLED_OFF; }
                else { BT_UnloaderControl.Text = "ON"; BT_UnloaderControl.Image = Properties.Resources.Yellow_LED; }
                ...
                if(mc.para.ETC.useHeadMode.value == (int)UnitCodeHead.HD1) BT_USE_HEAD.Text = BT_HEAD_ONLY_1.Text;
                else if (mc.para.ETC.useHeadMode.value == (int)UnitCodeHead.HD2) BT_USE_HEAD.Text = BT_HEAD_ONLY_2.Text;
                else if (mc.para.ETC.useHeadMode.value == (int)UnitCodeHead.HD_MAX) BT_USE_HEAD.Text = BT_HEAD_DUAL.Text;
		...
		private void Control_Click(object sender, EventArgs e)
		{
            ...
            if (sender.Equals(BT_HEAD_ONLY_1)) mc.para.ETC.useHeadMode.value = (int)UnitCodeHead.HD1;
            if (sender.Equals(BT_HEAD_ONLY_2)) mc.para.ETC.useHeadMode.value = (int)UnitCodeHead.HD2;
            if (sender.Equals(BT_HEAD_DUAL)) mc.para.ETC.useHeadMode.value = (int)UnitCodeHead.HD_MAX;
		...
		public void parameterRecipeOpenDialog()
		{
			...
			if (rst == DialogResult.OK)
			{
				try
				{
                    ...
                    {
                        if (!mc.hd.tool.Z[i].config.read(selected)) { EVENT.userDialogMessage(DIAG_SEL_MODE.OK, DIAG_ICON_MODE.FAILURE, "Gantry Z Motion Parameter Read ERROR!" + " #" + i.ToString()); return; }
                        if (!mc.hd.tool.T[i].config.read(selected)) { EVENT.userDialogMessage(DIAG_SEL_MODE.OK, DIAG_ICON_MODE.FAILURE, "Gantry T Motion Parameter Read ERROR!" + " #" + i.ToString()); return; }
                    }
		...
		public void parameterRecipeSaveDialog()
		{
			...
			if (rst == DialogResult.OK)
			{
				try
				{
                    ...
                    for (int i = 0; i < mc.activate.headCnt; i++)
                    {
                        if (!mc.hd.tool.Z[i].config.write(selected)) { EVENT.userDialogMessage(DIAG_SEL_MODE.OK, DIAG_ICON_MODE.FAILURE, "Gantry Z Motion Parameter Write ERROR!"); return; }
                        if (!mc.hd.tool.T[i].config.write(selected)) { EVENT.userDialogMessage(DIAG_SEL_MODE.OK, DIAG_ICON_MODE.FAILURE, "Gantry T Motion Parameter Write ERROR!"); return; }
                    }
