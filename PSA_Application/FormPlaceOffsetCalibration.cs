...
namespace PSA_Application
{
	public partial class FormPlaceOffsetCalibration : Form
	{
        ...
        static para_member[,] placeOffsetC1 = new para_member[50, 20];
        static para_member[,] placeOffsetC2 = new para_member[50, 20];
        static para_member[,] placeOffsetC3 = new para_member[50, 20];
        static para_member[,] placeOffsetC4 = new para_member[50, 20];
                ...
                while (!reqThreadStop)
                {
                    for (int i = 0; i < (int)mc.para.MT.padCount.x.value; i++)
                    {
                        ...
                        for (int j = 0; j < (int)mc.para.MT.padCount.y.value; j++)
                        {
                            ...
                            // 1. C1 이동 및 측정
                            posX = mc.hd.tool.lPos.x.PEDC1(i, true);
                            posY = mc.hd.tool.lPos.y.PEDC1(j, true);
                            ...
                            placeOffsetC1[0, 0].value = ret.d * 1000;
                            
                            // 2. C2 이동 및 측정
                            posX = mc.hd.tool.lPos.x.PEDC2(i, true);
                            posY = mc.hd.tool.lPos.y.PEDC2(j, true);
                            mc.hd.tool.jogMove(posX, posY, out ret.message); if (ret.message != RetMessage.OK) { mc.message.alarmMotion(ret.message); reqThreadStop = true; break; }

                            mc.idle(300);
                            ret.d = mc.AIN.Laser(); if (ret.d < -10) ret.d = -1;
                            laserResult = Math.Round(ret.d, 3);
                            placeOffsetC2[0, 0].value = ret.d * 1000;
                            // 3. C3 이동 및 측정
                            posX = mc.hd.tool.lPos.x.PEDC3(i, true);
                            posY = mc.hd.tool.lPos.y.PEDC3(j, true);
                            mc.hd.tool.jogMove(posX, posY, out ret.message); if (ret.message != RetMessage.OK) { mc.message.alarmMotion(ret.message); reqThreadStop = true; break; }

                            mc.idle(300);
                            ret.d = mc.AIN.Laser(); if (ret.d < -10) ret.d = -1;
                            laserResult = Math.Round(ret.d, 3);
                            placeOffsetC3[0, 0].value = ret.d * 1000;
                            // 4. C4 이동 및 측정
                            posX = mc.hd.tool.lPos.x.PEDC4(i, true);
                            posY = mc.hd.tool.lPos.y.PEDC4(j, true);
                            mc.hd.tool.jogMove(posX, posY, out ret.message); if (ret.message != RetMessage.OK) { mc.message.alarmMotion(ret.message); reqThreadStop = true; break; }

                            mc.idle(300);
                            ret.d = mc.AIN.Laser(); if (ret.d < -10) ret.d = -1;
                            laserResult = Math.Round(ret.d, 3);
                            placeOffsetC4[0, 0].value = ret.d * 1000;
                            ...
                            if (i == 0 && j == 0)
                            {
                                placeOffsetResultZ[0, 0].value = (placeOffsetC1[0, 0].value + placeOffsetC2[0, 0].value + placeOffsetC3[0, 0].value + placeOffsetC4[0, 0].value) / 4;
                            }
                            else
                            {
                                
                                placeOffsetResultZ[i, j].value = Math.Round((placeOffsetC1[0, 0].value
                                    + placeOffsetC2[0, 0].value
                                    + placeOffsetC3[0, 0].value
                                    + placeOffsetC4[0, 0].value) / 4 
                                    - placeOffsetResultZ[0, 0].value, 1);
                            }
                        }
                    }
		...
		private void Control_Click(object sender, EventArgs e)
		{
			...
			if (sender.Equals(BT_Set))
			{
				...
				for (int ix = 0; ix < 50; ix++)
				{
					for (int iy = 0; iy < 20; iy++)
					{
                        ...
                        mc.para.CAL.PDHeightC1[ix, iy].value = placeOffsetC1[ix, iy].value;
                        mc.para.CAL.PDHeightC2[ix, iy].value = placeOffsetC2[ix, iy].value;
                        mc.para.CAL.PDHeightC3[ix, iy].value = placeOffsetC3[ix, iy].value;
                        mc.para.CAL.PDHeightC4[ix, iy].value = placeOffsetC4[ix, iy].value;
					}
				}
				goto EXIT;
			LIMIT_OVER:
            ...
            if (sender.Equals(BT_Auto))
			{
				if (CB_ZHeight.Checked)
				{
                    ...
                    mc.pd.PD_Mode(PedestalMode.PD_UP, out ret.message);
