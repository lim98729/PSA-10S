...
namespace PSA_Application
{
	public partial class FormStart : Form
	{
		...
		void activate()
        {
            ...
            mc.activate.SYSTEM(out str); progressBar_Value += 10; mc.idle(idleDisplayTime);
            ...
            mc.activate.IO(out str); progressBar_Value += 5; mc.idle(idleDisplayTime);
            Display_TB_Message(str + "\n");

            Display_TB_Message("Analog Module Check ...");
            mc.activate.AXT(out str); progressBar_Value += 5; mc.idle(idleDisplayTime);
            Display_TB_Message(str + "\n");

            Display_TB_Message("Coor Module Check ...");
            mc.activate.coor(out str); progressBar_Value += 5; mc.idle(idleDisplayTime);
            Display_TB_Message(str + "\n");
            ...
            mc.activate.HDC(out str); progressBar_Value += 10; mc.idle(idleDisplayTime);
            if (mc.activate.success.HDC) Display_TB_Message("OK" + "\n");
            else Display_TB_Message(str + "\n");

            Display_TB_Message("Up-Looking Camera Check ...");
            mc.activate.ULC(out str);
            if (mc.para.ETC.unloaderControl.value == 0) progressBar_Value += 20;
            else progressBar_Value += 10; 
            mc.idle(idleDisplayTime);
            if (mc.activate.success.ULC) Display_TB_Message("OK" + "\n");
            else Display_TB_Message(str + "\n");

            if (mc.para.ETC.unloaderControl.value == 1)
            {
                Display_TB_Message("Pusher Check ...");
                mc.activate.PS(out str); progressBar_Value += 5; mc.idle(500);
                if (mc.activate.success.PS) Display_TB_Message("OK" + "\n");
                else Display_TB_Message(str + "\n");

                Display_TB_Message("Magazine Check ...");
                mc.activate.MG(out str); progressBar_Value += 5; mc.idle(500);
                if (mc.activate.success.MG) Display_TB_Message("OK" + "\n");
                else Display_TB_Message(str + "\n");
            }

            if (mc.activate.success.ALL || UtilityControl.simulation)
            {
                mc.OUT.MAIN.SAFETY(out ret.b1, out ret.message);
                if (!ret.b1) mc.OUT.MAIN.SAFETY(true, out ret.message);
