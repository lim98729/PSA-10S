	...
	public partial class CenterRight_Main : UserControl
	{
		...
		void refresh()
		{
			...
			else
			{
                ...
                else BT_PlaceForceCheck.Text = BT_PlaceForceCheck_Alarm.Text;
                ...
                if (mc.para.ETC.useAutoTiltCheck.value == 0) { BT_USE_AUTO_TILT_CHECK.Text = "OFF"; BT_USE_AUTO_TILT_CHECK.Image = Properties.Resources.YellowLED_OFF; }
                else { BT_USE_AUTO_TILT_CHECK.Text = "ON"; BT_USE_AUTO_TILT_CHECK.Image = Properties.Resources.Yellow_LED; }
		...
		private void Control_Click(object sender, EventArgs e)
		{
			...
			if (sender.Equals(BT_ForceCompenTest))
			{
                ...
                //double posZ = mc.hd.tool.tPos.z[0].XY_MOVING;
                ...
                //#region Z moving
                ////mc.idle(100);
                //posZ = mc.hd.tool.tPos.z[0].LOADCELL + 1000;   // 1mm 위치까지 이동
                //mc.hd.tool.jogMove(0, posZ, out ret.message); if (ret.message != RetMessage.OK) { mc.message.alarmMotion(ret.message); goto EXIT; }
                //mc.idle(100);
                //posZ = mc.hd.tool.tPos.z[0].LOADCELL;          // 살짝 접촉
                //mc.hd.tool.jogMove(0, posZ, out ret.message); if (ret.message != RetMessage.OK) { mc.message.alarmMotion(ret.message); goto EXIT; }
			...
			if (sender.Equals(BT_FlatCompenTest))
			{
				...
				else
				{
                    ...
                    mc.hd.tool.singleCycleHead = Convert.ToInt32(mc.para.ETC.useHeadMode.value);
                    mc.hd.reqMode = REQMODE.COMPEN_FLAT;
					mc.hd.req = true;
            ...
            if (sender.Equals(BT_USE_AUTO_TILT_CHECK))
            {
                if ((int)mc.para.ETC.useAutoTiltCheck.value == 0)
                    mc.para.setting(ref mc.para.ETC.useAutoTiltCheck, 1);
                else
                    mc.para.setting(ref mc.para.ETC.useAutoTiltCheck, 0);
            }
