...
namespace PSA_Application
{
	public partial class BottomLeft : UserControl
	{
		...
		private void BT_ErrorReset_Click(object sender, EventArgs e)
		{
            ...
            mc.hd.tool.Z[(int)UnitCodeHead.HD1].clearFault(out ret.message);
            mc.hd.tool.Z[(int)UnitCodeHead.HD2].clearFault(out ret.message);
            mc.hd.tool.T[(int)UnitCodeHead.HD1].clearFault(out ret.message);
            mc.hd.tool.T[(int)UnitCodeHead.HD2].clearFault(out ret.message);
            //mc.pd.X.clearFault(out ret.message);
            //mc.pd.Y.clearFault(out ret.message);
		}
...
*/
		...
		private void button1_Click(object sender, EventArgs e)
		{
            ...
            //int rndMin, rndMax;
            //int index = 0;
            //while (dwell.Elapsed < 1000)
            //{
            //    rndMin = (int)dwell.Elapsed - 200;
            //    rndMax = (int)dwell.Elapsed + 200;
            //    index++;
            //    EVENT.addLoadcellData2(0, dwell.Elapsed, random.Next(rndMin, rndMax) / 1000.0);//2, random.Next(rndMin, rndMax) / 1000.0);
