...
//                 for (int i = 1; i < 5; i++) tmp[i] = 0;			// 계속 0으로 초기화 시킬 필요가 있나..?
        ...
        void control(object sender, out RetMessage retMessage)
        {
            ...
            #region xyz moving
            mc.hd.tool.jogMove(selectedHead, posX + distanceX, posY + distanceY, mc.hd.tool.tPos.z[selectedHead].TOUCHPROBE, mc.hd.tool.tPos.t[selectedHead].ZERO, out retMessage); if (retMessage != RetMessage.OK) { mc.message.alarmMotion(ret.message); return; }
            #endregion
            mc.idle(1000);
            ...
            errorPos /= 1000;
            if (sender.Equals(BT_UP)) { touchPosition[1] = /*touchPosition[0] - */value + errorPos; }
            if (sender.Equals(BT_LEFT)) { touchPosition[2] = /*touchPosition[0] - */value + errorPos; }
            if (sender.Equals(BT_CENTER)) { touchPosition[0] = ret.d; }
            if (sender.Equals(BT_RIGHT)) { touchPosition[3] = /*touchPosition[0] - */value + errorPos; }
            if (sender.Equals(BT_DOWN)) { touchPosition[4] = /*touchPosition[0] - */value + errorPos; }

        EXIT:
            mc.hd.tool.jogMove(selectedHead, mc.coor.MP.HD.Z.XY_MOVING.value, out ret.message); if (ret.message != RetMessage.OK) { mc.message.alarmMotion(ret.message); return; } 
		#region  주석
        //    #region z moving
        //    int i, ii;
        ...
        private void BT_AutoCalibration_Click(object sender, EventArgs e)
        {
            ...
            mc.hd.tool.jogMove(selectedHead, posX, posY, mc.hd.tool.tPos.z[selectedHead].TOUCHPROBE, mc.hd.tool.tPos.t[selectedHead].ZERO, out ret.message); if (ret.message != RetMessage.OK) { mc.message.alarmMotion(ret.message); return; }
			mc.idle(500);
		...
		public void GetVerticalHeight()
		{
            ...
            mc.hd.tool.jogMove(selectedHead, posX, posY - distanceY, mc.hd.tool.tPos.z[selectedHead].TOUCHPROBE, mc.hd.tool.tPos.t[selectedHead].ZERO, out ret.message); if (ret.message != RetMessage.OK) { mc.message.alarmMotion(ret.message); return; }
			mc.idle(500);
            ...
            mc.hd.tool.Z[selectedHead].errorPosition(out errorPos, out ret.message);
			errorPos /= 1000;
            touchPosition[1] = ret.d - touchPosition[selectedHead];
		...
		public void GetHorizentalHeight()
		{
            ...
            mc.hd.tool.jogMove(selectedHead, posX + distanceX, posY, mc.hd.tool.tPos.z[selectedHead].TOUCHPROBE, mc.hd.tool.tPos.t[selectedHead].ZERO, out ret.message); if (ret.message != RetMessage.OK) { mc.message.alarmMotion(ret.message); return; }
			mc.idle(500);
