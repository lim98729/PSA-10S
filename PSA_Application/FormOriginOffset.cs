...
namespace PSA_Application
{
	public partial class FormOriginOffset : Form
	{
		...
		private void FormOriginOffset_Load(object sender, EventArgs e)
		{
            ...
            TB_HO_GANTRY_Z.Text = mc.hd.tool.Z[0].config.homing.originOffset.ToString();
            TB_HO_GANTRY_T.Text = mc.hd.tool.T[0].config.homing.originOffset.ToString();
        ...
        private void Control_Click(object sender, EventArgs e)
        {
            if (sender.Equals(BT_HO_SAVE))
			{
				try
				{
					...
					mc.hd.tool.Z[0].config.homing.originOffset = dtemp[2];
                    mc.hd.tool.T[0].config.homing.originOffset = dtemp[3];
			
					mc.sf.Z2.config.homing.originOffset = dtemp[6];
					mc.sf.Z.config.homing.originOffset = dtemp[7];

					mc.cv.W.config.homing.originOffset = dtemp[8];
				}
				catch(Exception ex)
				{
