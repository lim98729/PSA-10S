...
namespace PSA_Application
{
	public partial class BottomRight_Conveyor : UserControl
	{
		...
		private void OUT_Click(object sender, EventArgs e)
		{
			...
			#region OUT
			if (sender.Equals(BT_OUT_SIDEPUSHER))
			{
                mc.OUT.CV.SIDE_PUSHER(out ret.b, out ret.message);
                if (ret.message == RetMessage.OK) mc.OUT.CV.SIDE_PUSHER(!ret.b, out ret.message);
			}
		...
		private void Feeding_Click(object sender, EventArgs e)
		{
            ...
            {
                if (mc.init.success.PS) mc.ps.clear();
                if (mc.init.success.MG) { mc.unloader.clear(); mc.unloader.Elev.clear(); }
            }
