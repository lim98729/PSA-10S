...
// 						}
				...
				case SQC.AUTO:			// Map Data와 Area Sensor 간 일치하는지 확인
					...
					if (mc.alarm.startAlarmStatus) { mc2.req = MC_REQ.STOP; sqc = SQC.STOP; break; }
					sqc++; break;
				case SQC.AUTO + 1:
                    ...
                    if (mc.para.ETC.unloaderControl.value == 1)
                    {
                        ...
                        mc.unloader.reqMode = REQMODE.AUTO;
                    }
			...
			switch (sqc)
			{
				...
				case 11:
                    ...
                    if (mc.full.reqMode != REQMODE.BYPASS)
                    {
                        ...
                        {
                            mc.hd.tool.Z[i].actualPosition(out ret.d1, out ret.message); if (mpiCheck(mc.hd.tool.Z[i].config.axisCode, sqc, ret.message)) for_break = true;
                            if ((mc.hd.tool.tPos.z[i].XY_MOVING - ret.d1) > 1000) for_break = true;
                        }
                        if (for_break) break;
                    }
