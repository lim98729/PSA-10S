...
namespace PSA_SystemLibrary
{
	public class classStackFeeder : CONTROL
	{
		...
		public void control()
		{
			...
			switch (sqc)
			{
                ...
                #region HOMING
                case SQC.HOMING:
                    if (dev.NotExistHW.ZMP) { mc.init.success.SF = true; sqc = SQC.STOP; break; }
                    mc.init.success.SF = false;
                    mpi.zmp0.MOTOR_GENERAL_OUT(9, 9, true, out ret.message);
                    mpi.zmp0.MOTOR_GENERAL_OUT((int)UnitCodeAxisNumber.SF_Z1, 9, true, out ret.message);
                    mpi.zmp0.MOTOR_GENERAL_OUT((int)UnitCodeAxisNumber.SF_Z2, 9, true, out ret.message);
                    Z.setPosition(0, out ret.message);
                    Z.setCommandPosition(0, out ret.message);
                    Z2.setPosition(0, out ret.message);
                    Z2.setCommandPosition(0, out ret.message);
				...
				case SQC.AUTO + 1:
					...
					if (moveSFZ)
					{
                        if (ret.b1 || ret.b2)
                        {
                            mc.log.debug.write(mc.log.CODE.INFO, String.Format(textResource.LOG_DEBUG_SF_READY_UP, "MGZ #1 도착"), false);
                            workingZAxis = 1;
                            Z.stop(out ret.message);
                            Z2.stop(out ret.message);
                            Z2.reset(out ret.message);
	...
	public class classStackFeederPositionX
	{
		public double TUBE(UnitCodeSF unitCode)
		{
			...
			{
                if (unitCode == UnitCodeSF.SF1) tmp = mc.coor.MP.SF.X.TUBE1.value;
                else if (unitCode == UnitCodeSF.SF2) tmp = mc.coor.MP.SF.X.TUBE2.value;
                else if (unitCode == UnitCodeSF.SF3) tmp = mc.coor.MP.SF.X.TUBE3.value;
                else if (unitCode == UnitCodeSF.SF4) tmp = mc.coor.MP.SF.X.TUBE4.value;
                else if (unitCode == UnitCodeSF.SF5) tmp = mc.coor.MP.SF.X.TUBE5.value;
                else if (unitCode == UnitCodeSF.SF6) tmp = mc.coor.MP.SF.X.TUBE6.value;
                else if (unitCode == UnitCodeSF.SF7) tmp = mc.coor.MP.SF.X.TUBE7.value;
                else if (unitCode == UnitCodeSF.SF8) tmp = mc.coor.MP.SF.X.TUBE8.value;
                else tmp = mc.coor.MP.SF.X.TUBE1.value;
			}
			else
			{
                if (unitCode == UnitCodeSF.SF1) tmp = mc.coor.MP.SF.X_4SLOT.TUBE1.value;
				else if (unitCode == UnitCodeSF.SF2) tmp = mc.coor.MP.SF.X_4SLOT.TUBE2.value;
				else if (unitCode == UnitCodeSF.SF5) tmp = mc.coor.MP.SF.X_4SLOT.TUBE3.value;
				else if (unitCode == UnitCodeSF.SF6) tmp = mc.coor.MP.SF.X_4SLOT.TUBE4.value;
				else tmp = mc.coor.MP.SF.X_4SLOT.TUBE1.value;
			}
			return tmp;
		}
	}
	...
	public class classStackFeederPositionZ
	{
		public double DOWN
		{
			...
			{
				double tmp;
                tmp = (mc.swcontrol.mechanicalRevision == 0) ? mc.coor.MP.SF.Z.DOWN.value : mc.coor.MP.SF.Z.DOWN_4SLOT.value;		// 간섭 때문에 더 내려야 한다.
				return tmp;
			}
		}
		public double FULL_STROKE
		{
			...
			{
				double tmp;
                tmp = (mc.swcontrol.mechanicalRevision == 0) ? mc.coor.MP.SF.Z.STROKE.value : mc.coor.MP.SF.Z.STROKE_4SLOT.value;
				return tmp;
			}
		}
	}
