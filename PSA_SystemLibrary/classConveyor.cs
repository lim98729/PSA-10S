...
namespace PSA_SystemLibrary
{
	public class classConveyor : CONTROL
	{
		...
		public void control()
		{
			...
			switch (sqc)
			{
				...
				#region HOMING
				case SQC.HOMING:
                    if (dev.NotExistHW.ZMP) { mc.init.success.CV = true; sqc = SQC.STOP; break; }
                    mpi.zmp0.MOTOR_GENERAL_OUT(9, 9, true, out ret.message);
                    mpi.zmp0.MOTOR_GENERAL_OUT((int)UnitCodeAxisNumber.CV_W, 9, true, out ret.message);
					bool bdexist;
					mc.init.success.CV = false;
                    ...
                    W.setPosition(0, out ret.message);
                    W.setCommandPosition(0, out ret.message);
					sqc++; break;
				case SQC.HOMING + 1:
		...
		public void control()
		{
			...
			switch (sqc)
			{
                ...
                case 101:
                    ...
                    // Clamp 를 False 한다.
                    mc.OUT.CV.SIDE_PUSHER(false, out ret.message); if (ioCheck(sqc, ret.message)) break;
                    dwell.Reset();
                    sqc++; break;
                case 102:
				...
				case 201:
					...
					mc.OUT.CV.SIDE_PUSHER(true, out ret.message); if (ioCheck(sqc, ret.message)) break;
					dwell.Reset();
					sqc++; break;
				case 202:
					if (dwell.Elapsed < 100) break;
	...
	public class classConveyorToNextMC : CONTROL
	{
		...
		public void control()
		{
			...
			switch (sqc)
			{
				...
				case 200:
                    if (mc.para.ETC.unloaderControl.value == 1) { sqc += 2; break; }
					mc.IN.CV.BD_OUT(out ret.b, out ret.message);
					...
					if (dwell.Elapsed > 10000) 
                    {
                        if (ret.b)
                        {
                            errorCheck(ERRORCODE.CV, sqc, "", ALARM_CODE.E_CONV_NEXTMACH_TRAY_UNLOAD_TIMEOUT_SENSOR); break;
                        }
                    }
					if (dwell.Elapsed > 20000) { errorCheck(ERRORCODE.CV, sqc, "", ALARM_CODE.E_CONV_NEXTMACH_TRAY_UNLOAD_TIMEOUT_SMEMA); break; }
					break;
				case 201:
	...
	public class classConveyorPositionW
	{
		public double READY
		{
			...
			{
				double tmp;
                tmp = mc.coor.MP.CV.W.READY.value;
				return tmp;
			}
		}
		public double WIDTH
