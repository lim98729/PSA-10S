...
namespace PSA_SystemLibrary
{
    ...
    // 20160521. jhlim : 중요하지 않은 것들은 이걸로 홈 잡자..
    public class sensorHoming : CONTROL
    {
        mpiMotion Axis = new mpiMotion();
        ...
        public void control()
        {
            ...
            switch (sqc)
            {
                case 0:
                    if (dev.NotExistHW.ZMP) { sqc = SQC.STOP; break; }
                    Axis.setPosition(0, out ret.message); if (mpiCheck(Axis.config.axisCode, sqc, ret.message)) break;
                    Axis.setCommandPosition(0, out ret.message); if (mpiCheck(Axis.config.axisCode, sqc, ret.message)) break;
                    Esqc = 0; skip = false;
                    sqc++; break;
                ...
                case 21:
                    ...
                    Axis.movePlus(Axis.config.homing.homePosition, Axis.config.homing.speed, out ret.message); if (mpiCheck(Axis.config.axisCode, sqc, ret.message)) break;

                    dwell.Reset();
                    sqc++; break;
                case 22:
                    Axis.AT_TARGET(out ret.b, out ret.message);
                    if(!ret.b) break;
                    sqc++; break;
                case 23:
                    Axis.AT_DONE(out ret.b, out ret.message);
                    if(!ret.b) break;
                    Axis.AT_ERROR(out ret.b1, out ret.message);
                    if (ret.b1)
                    {
                        Axis.checkAlarmStatus(out ret.s, out ret.message);
                        errorCheck(Axis.config.axisNumber, ERRORCODE.HOMING, sqc, ret.s, ALARM_CODE.E_AXIS_NOT_IDLE_AFTER_RESET);
                    ...
                    Axis.P_LimitEventConfig(Axis.config.homing.P_LimitAction, Axis.config.homing.P_LimitPolarity, Axis.config.homing.P_LimitDuration, out ret.message); if (mpiCheck(Axis.config.axisCode, sqc, ret.message)) break;
					Axis.N_LimitEventConfig(Axis.config.homing.N_LimitAction, Axis.config.homing.N_LimitPolarity, Axis.config.homing.N_LimitDuration, out ret.message); if (mpiCheck(Axis.config.axisCode, sqc, ret.message)) break;
                    Axis.setPosition(0, out ret.message);
                    Axis.setCommandPosition(0, out ret.message);
                    sqc = SQC.STOP; break;
	...
	public class cameraTrigger
	{
        ...
        public void activate(mpiNodeDigtalOut _trigger)
        {
            ioType = IO_TYPE.GMS;
            tirggerGms = _trigger;
            isActivate = true;
		...
		public void output(bool cmd, out RetMessage retMessage)
		{
            ...
            if (ioType == IO_TYPE.GMS)
            {
                //mpi.zmp0.NODE_DIGITAL_SEG_OUT(8, 1, 30, cmd, out retMessage); if (retMessage != RetMessage.OK) return;
                mpi.zmp0.NODE_DIGITAL_SEG_OUT(tirggerGms.nodeNumber, tirggerGms.segNumber, tirggerGms.bitNumber, cmd, out retMessage); if (retMessage != RetMessage.OK) return;
                retMessage = RetMessage.OK;
                return;
            }
	...
	public class headParameter
	{
        ...
        public para_member headNumber; //LJS 필요에따라 삭제하기. 정진영대리님이쓰신것중에 쓰이는것으로 대체가능 
		...
		public void write(out bool r, string savepath = "C:\\PROTEC\\Data")
		{
			try
			{
                ...
                writeTuple(pick.pickupVacLimit[0], i, out i);
                writeTuple(pick.pickupVacLimit[1], i, out i);
                ...
                writeTuple(place.offset2.x, i, out i);
                writeTuple(place.offset2.y, i, out i);
                writeTuple(place.offset2.z, i, out i);
                writeTuple(place.offset2.t, i, out i);
                ...
                writeTuple(place.forceTracking.enable, i, out i);
                writeTuple(place.forceTracking.force, i, out i);
                writeTuple(place.forceTracking.level, i, out i);
                writeTuple(place.forceTracking.vel, i, out i);
                writeTuple(place.forceTracking.acc, i, out i);
                writeTuple(place.forceTracking.delay, i, out i);
				...
				writeTuple(place.placeForceOffset[0], i, out i);
                writeTuple(place.placeForceOffset[1], i, out i);
				#endregion
				#region press
		...
		public void upData(out bool r)
		{
			try
			{
                ...
                readTuple("pick.pickupVacLimit[0]", out pick.pickupVacLimit[0], out fail); if (fail) goto SET_FAIL;
                readTuple("pick.pickupVacLimit[1]", out pick.pickupVacLimit[1], out fail); if (fail) goto SET_FAIL;
                ...
                readTuple("place.offset2.x", out place.offset2.x, out fail); if (fail) goto SET_FAIL;
                readTuple("place.offset2.y", out place.offset2.y, out fail); if (fail) goto SET_FAIL;
                readTuple("place.offset2.z", out place.offset2.z, out fail); if (fail) goto SET_FAIL;
                readTuple("place.offset2.t", out place.offset2.t, out fail); if (fail) goto SET_FAIL;
                ...
                readTuple("place.forceTracking.enable", out place.forceTracking.enable, out fail); if (fail) goto SET_FAIL;
                readTuple("place.forceTracking.force", out place.forceTracking.force, out fail); if (fail) goto SET_FAIL;
                readTuple("place.forceTracking.level", out place.forceTracking.level, out fail); if (fail) goto SET_FAIL;
                readTuple("place.forceTracking.vel", out place.forceTracking.vel, out fail); if (fail) goto SET_FAIL;
                readTuple("place.forceTracking.acc", out place.forceTracking.acc, out fail); if (fail) goto SET_FAIL;
                readTuple("place.forceTracking.delay", out place.forceTracking.delay, out fail); if (fail) goto SET_FAIL;
                ...
                readTuple("place.placeForceOffset[0]", out place.placeForceOffset[0], out fail); if (fail) goto SET_FAIL;
                readTuple("place.placeForceOffset[1]", out place.placeForceOffset[1], out fail); if (fail) goto SET_FAIL;
		...
		void readTuple(string paraName, out para_member p, out bool fail)
		{
			HTuple i;
			fail = false;
		...
		void setDefault(out bool r)
		{
            ...
            setDefault("pick.pickupVacLimit[0]", out pick.pickupVacLimit[0], out fail); if (fail) goto SET_FAIL;
            setDefault("pick.pickupVacLimit[1]", out pick.pickupVacLimit[1], out fail); if (fail) goto SET_FAIL;
            ...
            setDefault("place.offset2.x", out place.offset2.x, out fail); if (fail) goto SET_FAIL;
            setDefault("place.offset2.y", out place.offset2.y, out fail); if (fail) goto SET_FAIL;
            setDefault("place.offset2.z", out place.offset2.z, out fail); if (fail) goto SET_FAIL;
            setDefault("place.offset2.t", out place.offset2.t, out fail); if (fail) goto SET_FAIL;
            ...
            setDefault("place.forceTracking.enable", out place.forceTracking.enable, out fail); if (fail) goto SET_FAIL;
            setDefault("place.forceTracking.force", out place.forceTracking.force, out fail); if (fail) goto SET_FAIL;
            setDefault("place.forceTracking.level", out place.forceTracking.level, out fail); if (fail) goto SET_FAIL;
            setDefault("place.forceTracking.vel", out place.forceTracking.vel, out fail); if (fail) goto SET_FAIL;
            setDefault("place.forceTracking.acc", out place.forceTracking.acc, out fail); if (fail) goto SET_FAIL;
            setDefault("place.forceTracking.delay", out place.forceTracking.delay, out fail); if (fail) goto SET_FAIL;
			...
			setDefault("place.placeForceOffset[0]", out place.placeForceOffset[0], out fail); if (fail) goto SET_FAIL;
            setDefault("place.placeForceOffset[1]", out place.placeForceOffset[1], out fail); if (fail) goto SET_FAIL;
			#endregion
			#region press
		...
		void setDefault(HTuple name, out para_member p, out bool fail)
		{
			try
			{
                ...
                id++; if (name == "pick.pickupVacLimit[0]") setDefault(out p, name, id, 0, -5000, 5000, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "pick.pickupVacLimit[1]") setDefault(out p, name, id, 0, -5000, 5000, AUTHORITY.MAINTENCE.ToString(), "description");
                ...
                id++; if (name == "place.offset2.x") setDefault(out p, name, id, 0, -5000, 5000, AUTHORITY.MAINTENCE.ToString(), "Place X Position Offset[um]");
                id++; if (name == "place.offset2.y") setDefault(out p, name, id, 0, -5000, 5000, AUTHORITY.MAINTENCE.ToString(), "Place Y Position Offset[um]");
                id++; if (name == "place.offset2.z") setDefault(out p, name, id, 0, -5000, 5000, AUTHORITY.MAINTENCE.ToString(), "Place Z Position Offset[um]");
                id++; if (name == "place.offset2.t") setDefault(out p, name, id, 0, -5, 5, AUTHORITY.MAINTENCE.ToString(), "Place T Angle Offset[Degree]");
                ...
                id++; if (name == "place.forceTracking.enable") setDefault(out p, name, id, 1, 0, 1, AUTHORITY.MAINTENCE.ToString(), "Enable Force Tracking");
                id++; if (name == "place.forceTracking.force") setDefault(out p, name, id, 0, 0, 15, AUTHORITY.MAINTENCE.ToString(), "Force for Tracking[kg]");
                id++; if (name == "place.forceTracking.level") setDefault(out p, name, id, 1000, 100, 3000, AUTHORITY.MAINTENCE.ToString(), "2nd Search Target Z Offset Value before Placing[um]");
                id++; if (name == "place.forceTracking.vel") setDefault(out p, name, id, 0.1, 0.000001, 50, AUTHORITY.MAINTENCE.ToString(), "Force Tracking Target Velocity before Placing[mm/sec]");
                id++; if (name == "place.forceTracking.acc") setDefault(out p, name, id, 0.1, 0.000001, 1, AUTHORITY.MAINTENCE.ToString(), "Force Tracking Target Acceleration Speed before Placing[m/sec^2]");
                id++; if (name == "place.forceTracking.delay") setDefault(out p, name, id, 0, 0, 5000, AUTHORITY.MAINTENCE.ToString(), "Delay Time after 2nd Search Motion Done in Placing[mSec]");
				...
				id++; if (name == "place.suction.mode") setDefault(out p, name, id, 0, 0, 2, AUTHORITY.MAINTENCE.ToString(), "Suction Off Timing Selection in Place. Search/Place");
				...
				id++; if (name == "place.placeForceOffset[0]") setDefault(out p, name, id, 0, -10000, 10000, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "place.placeForceOffset[1]") setDefault(out p, name, id, 0, -10000, 10000, AUTHORITY.MAINTENCE.ToString(), "description");
				#endregion
				#region press
	...
	public class headCamearaParameter
	{
		...
		public void write(out bool r, string savepath = "C:\\PROTEC\\Data")
		{
			try
			{
                ...
                writeTuple(modelPAD.quardrant, i, out i);
                writeTuple(modelPAD.proj_EdgeFilter, i, out i);
                writeTuple(modelPAD.proj_Direction, i, out i);
                writeTuple(modelPAD.proj_Type, i, out i);
                writeTuple(modelPAD.proj_MinTh, i, out i);
                writeTuple(modelPAD.proj_MaxTh, i, out i);
                writeTuple(modelPAD.patternPos.x, i, out i);
                writeTuple(modelPAD.patternPos.y, i, out i);
                ...
                writeTuple(modelPADC1.quardrant, i, out i);
                writeTuple(modelPADC1.proj_EdgeFilter, i, out i);
                writeTuple(modelPADC1.proj_Direction, i, out i);
                writeTuple(modelPADC1.proj_Type, i, out i);
                writeTuple(modelPADC1.proj_MinTh, i, out i);
                writeTuple(modelPADC1.proj_MaxTh, i, out i);
                writeTuple(modelPADC1.patternPos.x, i, out i);
                writeTuple(modelPADC1.patternPos.y, i, out i);
                ...
                writeTuple(modelPADC2.quardrant, i, out i);
                writeTuple(modelPADC2.proj_EdgeFilter, i, out i);
                writeTuple(modelPADC2.proj_Direction, i, out i);
                writeTuple(modelPADC2.proj_Type, i, out i);
                writeTuple(modelPADC2.proj_MinTh, i, out i);
                writeTuple(modelPADC2.proj_MaxTh, i, out i);
                writeTuple(modelPADC2.patternPos.x, i, out i);
                writeTuple(modelPADC2.patternPos.y, i, out i);
                ...
                writeTuple(modelPADC3.quardrant, i, out i);
                writeTuple(modelPADC3.proj_EdgeFilter, i, out i);
                writeTuple(modelPADC3.proj_Direction, i, out i);
                writeTuple(modelPADC3.proj_Type, i, out i);
                writeTuple(modelPADC3.proj_MinTh, i, out i);
                writeTuple(modelPADC3.proj_MaxTh, i, out i);
                writeTuple(modelPADC3.patternPos.x, i, out i);
                writeTuple(modelPADC3.patternPos.y, i, out i);
                ...
                writeTuple(modelPADC4.quardrant, i, out i);
                writeTuple(modelPADC4.proj_EdgeFilter, i, out i);
                writeTuple(modelPADC4.proj_Direction, i, out i);
                writeTuple(modelPADC4.proj_Type, i, out i);
                writeTuple(modelPADC4.proj_MinTh, i, out i);
                writeTuple(modelPADC4.proj_MaxTh, i, out i);
                writeTuple(modelPADC4.patternPos.x, i, out i);
                writeTuple(modelPADC4.patternPos.y, i, out i);
		...
		public void upData(out bool r)
		{
			try
			{
                ...
                readTuple("modelPAD.quardrant", out modelPAD.quardrant, out fail); if (fail) goto SET_FAIL;
                readTuple("modelPAD.proj_EdgeFilter", out modelPAD.proj_EdgeFilter, out fail); if (fail) goto SET_FAIL;
                readTuple("modelPAD.proj_Direction", out modelPAD.proj_Direction, out fail); if (fail) goto SET_FAIL;
                readTuple("modelPAD.proj_Type", out modelPAD.proj_Type, out fail); if (fail) goto SET_FAIL;
                readTuple("modelPAD.proj_MinTh", out modelPAD.proj_MinTh, out fail); if (fail) goto SET_FAIL;
                readTuple("modelPAD.proj_MaxTh", out modelPAD.proj_MaxTh, out fail); if (fail) goto SET_FAIL;
                readTuple("modelPAD.patternPos.x", out modelPAD.patternPos.x, out fail); if (fail) goto SET_FAIL;
                readTuple("modelPAD.patternPos.y", out modelPAD.patternPos.y, out fail); if (fail) goto SET_FAIL;
                ...
                readTuple("modelPADC1.quardrant", out modelPADC1.quardrant, out fail); if (fail) goto SET_FAIL;
                readTuple("modelPADC1.proj_EdgeFilter", out modelPADC1.proj_EdgeFilter, out fail); if (fail) goto SET_FAIL;
                readTuple("modelPADC1.proj_Direction", out modelPADC1.proj_Direction, out fail); if (fail) goto SET_FAIL;
                readTuple("modelPADC1.proj_Type", out modelPADC1.proj_Type, out fail); if (fail) goto SET_FAIL;
                readTuple("modelPADC1.proj_MinTh", out modelPADC1.proj_MinTh, out fail); if (fail) goto SET_FAIL;
                readTuple("modelPADC1.proj_MaxTh", out modelPADC1.proj_MaxTh, out fail); if (fail) goto SET_FAIL;
                readTuple("modelPADC1.patternPos.x", out modelPADC1.patternPos.x, out fail); if (fail) goto SET_FAIL;
                readTuple("modelPADC1.patternPos.y", out modelPADC1.patternPos.y, out fail); if (fail) goto SET_FAIL;
                ...
                readTuple("modelPADC2.quardrant", out modelPADC2.quardrant, out fail); if (fail) goto SET_FAIL;
                readTuple("modelPADC2.proj_EdgeFilter", out modelPADC2.proj_EdgeFilter, out fail); if (fail) goto SET_FAIL;
                readTuple("modelPADC2.proj_Direction", out modelPADC2.proj_Direction, out fail); if (fail) goto SET_FAIL;
                readTuple("modelPADC2.proj_Type", out modelPADC2.proj_Type, out fail); if (fail) goto SET_FAIL;
                readTuple("modelPADC2.proj_MinTh", out modelPADC2.proj_MinTh, out fail); if (fail) goto SET_FAIL;
                readTuple("modelPADC2.proj_MaxTh", out modelPADC2.proj_MaxTh, out fail); if (fail) goto SET_FAIL;
                readTuple("modelPADC2.patternPos.x", out modelPADC2.patternPos.x, out fail); if (fail) goto SET_FAIL;
                readTuple("modelPADC2.patternPos.y", out modelPADC2.patternPos.y, out fail); if (fail) goto SET_FAIL;
                ...
                readTuple("modelPADC3.quardrant", out modelPADC3.quardrant, out fail); if (fail) goto SET_FAIL;
                readTuple("modelPADC3.proj_EdgeFilter", out modelPADC3.proj_EdgeFilter, out fail); if (fail) goto SET_FAIL;
                readTuple("modelPADC3.proj_Direction", out modelPADC3.proj_Direction, out fail); if (fail) goto SET_FAIL;
                readTuple("modelPADC3.proj_Type", out modelPADC3.proj_Type, out fail); if (fail) goto SET_FAIL;
                readTuple("modelPADC3.proj_MinTh", out modelPADC3.proj_MinTh, out fail); if (fail) goto SET_FAIL;
                readTuple("modelPADC3.proj_MaxTh", out modelPADC3.proj_MaxTh, out fail); if (fail) goto SET_FAIL;
                readTuple("modelPADC3.patternPos.x", out modelPADC3.patternPos.x, out fail); if (fail) goto SET_FAIL;
                readTuple("modelPADC3.patternPos.y", out modelPADC3.patternPos.y, out fail); if (fail) goto SET_FAIL;
                ...
                readTuple("modelPADC4.quardrant", out modelPADC4.quardrant, out fail); if (fail) goto SET_FAIL;
                readTuple("modelPADC4.proj_EdgeFilter", out modelPADC4.proj_EdgeFilter, out fail); if (fail) goto SET_FAIL;
                readTuple("modelPADC4.proj_Direction", out modelPADC4.proj_Direction, out fail); if (fail) goto SET_FAIL;
                readTuple("modelPADC4.proj_Type", out modelPADC4.proj_Type, out fail); if (fail) goto SET_FAIL;
                readTuple("modelPADC4.proj_MinTh", out modelPADC4.proj_MinTh, out fail); if (fail) goto SET_FAIL;
                readTuple("modelPADC4.proj_MaxTh", out modelPADC4.proj_MaxTh, out fail); if (fail) goto SET_FAIL;
                readTuple("modelPADC4.patternPos.x", out modelPADC4.patternPos.x, out fail); if (fail) goto SET_FAIL;
                readTuple("modelPADC4.patternPos.y", out modelPADC4.patternPos.y, out fail); if (fail) goto SET_FAIL;
		...
		void setDefault(out bool r)
		{
            ...
            setDefault("modelPAD.quardrant", out modelPAD.quardrant, out fail); if (fail) goto SET_FAIL;
            setDefault("modelPAD.proj_EdgeFilter", out modelPAD.proj_EdgeFilter, out fail); if (fail) goto SET_FAIL;
            setDefault("modelPAD.proj_Direction", out modelPAD.proj_Direction, out fail); if (fail) goto SET_FAIL;
            setDefault("modelPAD.proj_Type", out modelPAD.proj_Type, out fail); if (fail) goto SET_FAIL;
            setDefault("modelPAD.proj_MinTh", out modelPAD.proj_MinTh, out fail); if (fail) goto SET_FAIL;
            setDefault("modelPAD.proj_MaxTh", out modelPAD.proj_MaxTh, out fail); if (fail) goto SET_FAIL;
            setDefault("modelPAD.patternPos.x", out modelPAD.patternPos.x, out fail); if (fail) goto SET_FAIL;
            setDefault("modelPAD.patternPos.y", out modelPAD.patternPos.y, out fail); if (fail) goto SET_FAIL;
            ...
            setDefault("modelPADC1.quardrant", out modelPADC1.quardrant, out fail); if (fail) goto SET_FAIL;
            setDefault("modelPADC1.proj_EdgeFilter", out modelPADC1.proj_EdgeFilter, out fail); if (fail) goto SET_FAIL;
            setDefault("modelPADC1.proj_Direction", out modelPADC1.proj_Direction, out fail); if (fail) goto SET_FAIL;
            setDefault("modelPADC1.proj_Type", out modelPADC1.proj_Type, out fail); if (fail) goto SET_FAIL;
            setDefault("modelPADC1.proj_MinTh", out modelPADC1.proj_MinTh, out fail); if (fail) goto SET_FAIL;
            setDefault("modelPADC1.proj_MaxTh", out modelPADC1.proj_MaxTh, out fail); if (fail) goto SET_FAIL;
            setDefault("modelPADC1.patternPos.x", out modelPADC1.patternPos.x, out fail); if (fail) goto SET_FAIL;
            setDefault("modelPADC1.patternPos.y", out modelPADC1.patternPos.y, out fail); if (fail) goto SET_FAIL;
            ...
            setDefault("modelPADC2.quardrant", out modelPADC2.quardrant, out fail); if (fail) goto SET_FAIL;
            setDefault("modelPADC2.proj_EdgeFilter", out modelPADC2.proj_EdgeFilter, out fail); if (fail) goto SET_FAIL;
            setDefault("modelPADC2.proj_Direction", out modelPADC2.proj_Direction, out fail); if (fail) goto SET_FAIL;
            setDefault("modelPADC2.proj_Type", out modelPADC2.proj_Type, out fail); if (fail) goto SET_FAIL;
            setDefault("modelPADC2.proj_MinTh", out modelPADC2.proj_MinTh, out fail); if (fail) goto SET_FAIL;
            setDefault("modelPADC2.proj_MaxTh", out modelPADC2.proj_MaxTh, out fail); if (fail) goto SET_FAIL;
            setDefault("modelPADC2.patternPos.x", out modelPADC2.patternPos.x, out fail); if (fail) goto SET_FAIL;
            setDefault("modelPADC2.patternPos.y", out modelPADC2.patternPos.y, out fail); if (fail) goto SET_FAIL;
            ...
            setDefault("modelPADC3.quardrant", out modelPADC3.quardrant, out fail); if (fail) goto SET_FAIL;
            setDefault("modelPADC3.proj_EdgeFilter", out modelPADC3.proj_EdgeFilter, out fail); if (fail) goto SET_FAIL;
            setDefault("modelPADC3.proj_Direction", out modelPADC3.proj_Direction, out fail); if (fail) goto SET_FAIL;
            setDefault("modelPADC3.proj_Type", out modelPADC3.proj_Type, out fail); if (fail) goto SET_FAIL;
            setDefault("modelPADC3.proj_MinTh", out modelPADC3.proj_MinTh, out fail); if (fail) goto SET_FAIL;
            setDefault("modelPADC3.proj_MaxTh", out modelPADC3.proj_MaxTh, out fail); if (fail) goto SET_FAIL;
            setDefault("modelPADC3.patternPos.x", out modelPADC3.patternPos.x, out fail); if (fail) goto SET_FAIL;
            setDefault("modelPADC3.patternPos.y", out modelPADC3.patternPos.y, out fail); if (fail) goto SET_FAIL;
            ...
            setDefault("modelPADC4.quardrant", out modelPADC4.quardrant, out fail); if (fail) goto SET_FAIL;
            setDefault("modelPADC4.proj_EdgeFilter", out modelPADC4.proj_EdgeFilter, out fail); if (fail) goto SET_FAIL;
            setDefault("modelPADC4.proj_Direction", out modelPADC4.proj_Direction, out fail); if (fail) goto SET_FAIL;
            setDefault("modelPADC4.proj_Type", out modelPADC4.proj_Type, out fail); if (fail) goto SET_FAIL;
            setDefault("modelPADC4.proj_MinTh", out modelPADC4.proj_MinTh, out fail); if (fail) goto SET_FAIL;
            setDefault("modelPADC4.proj_MaxTh", out modelPADC4.proj_MaxTh, out fail); if (fail) goto SET_FAIL;
            setDefault("modelPADC4.patternPos.x", out modelPADC4.patternPos.x, out fail); if (fail) goto SET_FAIL;
            setDefault("modelPADC4.patternPos.y", out modelPADC4.patternPos.y, out fail); if (fail) goto SET_FAIL;
		...
		void setDefault(HTuple name, out para_member p, out bool fail)
		{
			try
			{
                ...
                id++; if (name == "modelPAD.quardrant") setDefault(out p, name, id, 0, 0, 3, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelPAD.proj_EdgeFilter") setDefault(out p, name, id, 30, 0, 100, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelPAD.proj_Direction") setDefault(out p, name, id, 1, 0, 1, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelPAD.proj_Type") setDefault(out p, name, id, 0, 0, 1, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelPAD.proj_MinTh") setDefault(out p, name, id, 150, 0, 255, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelPAD.proj_MaxTh") setDefault(out p, name, id, 255, 0, 255, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelPAD.patternPos.x") setDefault(out p, name, id, 0, -100000, 100000, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelPAD.patternPos.y") setDefault(out p, name, id, 0, -100000, 100000, AUTHORITY.MAINTENCE.ToString(), "description");
                ...
                id++; if (name == "modelPADC1.quardrant") setDefault(out p, name, id, 0, 0, 3, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelPADC1.proj_EdgeFilter") setDefault(out p, name, id, 30, 0, 100, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelPADC1.proj_Direction") setDefault(out p, name, id, 1, 0, 1, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelPADC1.proj_Type") setDefault(out p, name, id, 0, 0, 1, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelPADC1.proj_MinTh") setDefault(out p, name, id, 150, 0, 255, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelPADC1.proj_MaxTh") setDefault(out p, name, id, 255, 0, 255, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelPADC1.patternPos.x") setDefault(out p, name, id, 0, -100000, 100000, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelPADC1.patternPos.y") setDefault(out p, name, id, 0, -100000, 100000, AUTHORITY.MAINTENCE.ToString(), "description");
                ...
                id++; if (name == "modelPADC2.quardrant") setDefault(out p, name, id, 0, 0, 3, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelPADC2.proj_EdgeFilter") setDefault(out p, name, id, 30, 0, 100, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelPADC2.proj_Direction") setDefault(out p, name, id, 1, 0, 1, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelPADC2.proj_Type") setDefault(out p, name, id, 0, 0, 1, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelPADC2.proj_MinTh") setDefault(out p, name, id, 150, 0, 255, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelPADC2.proj_MaxTh") setDefault(out p, name, id, 255, 0, 255, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelPADC2.patternPos.x") setDefault(out p, name, id, 0, -100000, 100000, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelPADC2.patternPos.y") setDefault(out p, name, id, 0, -100000, 100000, AUTHORITY.MAINTENCE.ToString(), "description");
                ...
                id++; if (name == "modelPADC3.quardrant") setDefault(out p, name, id, 0, 0, 3, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelPADC3.proj_EdgeFilter") setDefault(out p, name, id, 30, 0, 100, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelPADC3.proj_Direction") setDefault(out p, name, id, 1, 0, 1, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelPADC3.proj_Type") setDefault(out p, name, id, 0, 0, 1, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelPADC3.proj_MinTh") setDefault(out p, name, id, 150, 0, 255, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelPADC3.proj_MaxTh") setDefault(out p, name, id, 255, 0, 255, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelPADC3.patternPos.x") setDefault(out p, name, id, 0, -100000, 100000, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelPADC3.patternPos.y") setDefault(out p, name, id, 0, -100000, 100000, AUTHORITY.MAINTENCE.ToString(), "description");
                ...
                id++; if (name == "modelPADC4.quardrant") setDefault(out p, name, id, 0, 0, 3, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelPADC4.proj_EdgeFilter") setDefault(out p, name, id, 30, 0, 100, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelPADC4.proj_Direction") setDefault(out p, name, id, 1, 0, 1, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelPADC4.proj_Type") setDefault(out p, name, id, 0, 0, 1, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelPADC4.proj_MinTh") setDefault(out p, name, id, 150, 0, 255, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelPADC4.proj_MaxTh") setDefault(out p, name, id, 255, 0, 255, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelPADC4.patternPos.x") setDefault(out p, name, id, 0, -100000, 100000, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelPADC4.patternPos.y") setDefault(out p, name, id, 0, -100000, 100000, AUTHORITY.MAINTENCE.ToString(), "description");
	...
	public class upLookingCamearaParameter
	{
        ...
        public halconModelParameter modelCorner1 = new halconModelParameter();
        public halconModelParameter modelCorner2 = new halconModelParameter();
        public halconModelParameter modelCorner3 = new halconModelParameter();
        public halconModelParameter modelCorner4 = new halconModelParameter();
		...
		public void write(out bool r, string savepath = "C:\\PROTEC\\Data")
		{
			try
			{
                ...
                writeTuple(modelCorner1.isCreate, i, out i);
                writeTuple(modelCorner1.ID, i, out i);
                writeTuple(modelCorner1.algorism, i, out i);
                writeTuple(modelCorner1.passScore, i, out i);
                writeTuple(modelCorner1.angleStart, i, out i);
                writeTuple(modelCorner1.angleExtent, i, out i);
                writeTuple(modelCorner1.exposureTime, i, out i);
                writeTuple(modelCorner1.light.ch1, i, out i);
                writeTuple(modelCorner1.light.ch2, i, out i);

                writeTuple(modelCorner2.isCreate, i, out i);
                writeTuple(modelCorner2.ID, i, out i);
                writeTuple(modelCorner2.algorism, i, out i);
                writeTuple(modelCorner2.passScore, i, out i);
                writeTuple(modelCorner2.angleStart, i, out i);
                writeTuple(modelCorner2.angleExtent, i, out i);
                writeTuple(modelCorner2.exposureTime, i, out i);
                writeTuple(modelCorner2.light.ch1, i, out i);
                writeTuple(modelCorner2.light.ch2, i, out i);

                writeTuple(modelCorner3.isCreate, i, out i);
                writeTuple(modelCorner3.ID, i, out i);
                writeTuple(modelCorner3.algorism, i, out i);
                writeTuple(modelCorner3.passScore, i, out i);
                writeTuple(modelCorner3.angleStart, i, out i);
                writeTuple(modelCorner3.angleExtent, i, out i);
                writeTuple(modelCorner3.exposureTime, i, out i);
                writeTuple(modelCorner3.light.ch1, i, out i);
                writeTuple(modelCorner3.light.ch2, i, out i);

                writeTuple(modelCorner4.isCreate, i, out i);
                writeTuple(modelCorner4.ID, i, out i);
                writeTuple(modelCorner4.algorism, i, out i);
                writeTuple(modelCorner4.passScore, i, out i);
                writeTuple(modelCorner4.angleStart, i, out i);
                writeTuple(modelCorner4.angleExtent, i, out i);
                writeTuple(modelCorner4.exposureTime, i, out i);
                writeTuple(modelCorner4.light.ch1, i, out i);
                writeTuple(modelCorner4.light.ch2, i, out i);
		...
		public void upData(out bool r)
		{
			try
			{
                ...
                readTuple("modelCorner1.isCreate", out modelCorner1.isCreate, out fail); if (fail) goto SET_FAIL;
                readTuple("modelCorner1.ID", out modelCorner1.ID, out fail); if (fail) goto SET_FAIL;
                readTuple("modelCorner1.algorism", out modelCorner1.algorism, out fail); if (fail) goto SET_FAIL;
                readTuple("modelCorner1.passScore", out modelCorner1.passScore, out fail); if (fail) goto SET_FAIL;
                readTuple("modelCorner1.angleStart", out modelCorner1.angleStart, out fail); if (fail) goto SET_FAIL;
                readTuple("modelCorner1.angleExtent", out modelCorner1.angleExtent, out fail); if (fail) goto SET_FAIL;
                readTuple("modelCorner1.exposureTime", out modelCorner1.exposureTime, out fail); if (fail) goto SET_FAIL;
                readTuple("modelCorner1.light.ch1", out modelCorner1.light.ch1, out fail); if (fail) goto SET_FAIL;
                readTuple("modelCorner1.light.ch2", out modelCorner1.light.ch2, out fail); if (fail) goto SET_FAIL;

                readTuple("modelCorner2.isCreate", out modelCorner2.isCreate, out fail); if (fail) goto SET_FAIL;
                readTuple("modelCorner2.ID", out modelCorner2.ID, out fail); if (fail) goto SET_FAIL;
                readTuple("modelCorner2.algorism", out modelCorner2.algorism, out fail); if (fail) goto SET_FAIL;
                readTuple("modelCorner2.passScore", out modelCorner2.passScore, out fail); if (fail) goto SET_FAIL;
                readTuple("modelCorner2.angleStart", out modelCorner2.angleStart, out fail); if (fail) goto SET_FAIL;
                readTuple("modelCorner2.angleExtent", out modelCorner2.angleExtent, out fail); if (fail) goto SET_FAIL;
                readTuple("modelCorner2.exposureTime", out modelCorner2.exposureTime, out fail); if (fail) goto SET_FAIL;
                readTuple("modelCorner2.light.ch1", out modelCorner2.light.ch1, out fail); if (fail) goto SET_FAIL;
                readTuple("modelCorner2.light.ch2", out modelCorner2.light.ch2, out fail); if (fail) goto SET_FAIL;

                readTuple("modelCorner3.isCreate", out modelCorner3.isCreate, out fail); if (fail) goto SET_FAIL;
                readTuple("modelCorner3.ID", out modelCorner3.ID, out fail); if (fail) goto SET_FAIL;
                readTuple("modelCorner3.algorism", out modelCorner3.algorism, out fail); if (fail) goto SET_FAIL;
                readTuple("modelCorner3.passScore", out modelCorner3.passScore, out fail); if (fail) goto SET_FAIL;
                readTuple("modelCorner3.angleStart", out modelCorner3.angleStart, out fail); if (fail) goto SET_FAIL;
                readTuple("modelCorner3.angleExtent", out modelCorner3.angleExtent, out fail); if (fail) goto SET_FAIL;
                readTuple("modelCorner3.exposureTime", out modelCorner3.exposureTime, out fail); if (fail) goto SET_FAIL;
                readTuple("modelCorner3.light.ch1", out modelCorner3.light.ch1, out fail); if (fail) goto SET_FAIL;
                readTuple("modelCorner3.light.ch2", out modelCorner3.light.ch2, out fail); if (fail) goto SET_FAIL;

                readTuple("modelCorner4.isCreate", out modelCorner4.isCreate, out fail); if (fail) goto SET_FAIL;
                readTuple("modelCorner4.ID", out modelCorner4.ID, out fail); if (fail) goto SET_FAIL;
                readTuple("modelCorner4.algorism", out modelCorner4.algorism, out fail); if (fail) goto SET_FAIL;
                readTuple("modelCorner4.passScore", out modelCorner4.passScore, out fail); if (fail) goto SET_FAIL;
                readTuple("modelCorner4.angleStart", out modelCorner4.angleStart, out fail); if (fail) goto SET_FAIL;
                readTuple("modelCorner4.angleExtent", out modelCorner4.angleExtent, out fail); if (fail) goto SET_FAIL;
                readTuple("modelCorner4.exposureTime", out modelCorner4.exposureTime, out fail); if (fail) goto SET_FAIL;
                readTuple("modelCorner4.light.ch1", out modelCorner4.light.ch1, out fail); if (fail) goto SET_FAIL;
                readTuple("modelCorner4.light.ch2", out modelCorner4.light.ch2, out fail); if (fail) goto SET_FAIL;
		...
		void setDefault(out bool r)
		{
            ...
            setDefault("modelCorner1.isCreate", out modelCorner1.isCreate, out fail); if (fail) goto SET_FAIL;
            setDefault("modelCorner1.ID", out modelCorner1.ID, out fail); if (fail) goto SET_FAIL;
            setDefault("modelCorner1.algorism", out modelCorner1.algorism, out fail); if (fail) goto SET_FAIL;
            setDefault("modelCorner1.passScore", out modelCorner1.passScore, out fail); if (fail) goto SET_FAIL;
            setDefault("modelCorner1.angleStart", out modelCorner1.angleStart, out fail); if (fail) goto SET_FAIL;
            setDefault("modelCorner1.angleExtent", out modelCorner1.angleExtent, out fail); if (fail) goto SET_FAIL;
            setDefault("modelCorner1.exposureTime", out modelCorner1.exposureTime, out fail); if (fail) goto SET_FAIL;
            setDefault("modelCorner1.light.ch1", out modelCorner1.light.ch1, out fail); if (fail) goto SET_FAIL;
            setDefault("modelCorner1.light.ch2", out modelCorner1.light.ch2, out fail); if (fail) goto SET_FAIL;
          
            setDefault("modelCorner2.isCreate", out modelCorner2.isCreate, out fail); if (fail) goto SET_FAIL;
            setDefault("modelCorner2.ID", out modelCorner2.ID, out fail); if (fail) goto SET_FAIL;
            setDefault("modelCorner2.algorism", out modelCorner2.algorism, out fail); if (fail) goto SET_FAIL;
            setDefault("modelCorner2.passScore", out modelCorner2.passScore, out fail); if (fail) goto SET_FAIL;
            setDefault("modelCorner2.angleStart", out modelCorner2.angleStart, out fail); if (fail) goto SET_FAIL;
            setDefault("modelCorner2.angleExtent", out modelCorner2.angleExtent, out fail); if (fail) goto SET_FAIL;
            setDefault("modelCorner2.exposureTime", out modelCorner2.exposureTime, out fail); if (fail) goto SET_FAIL;
            setDefault("modelCorner2.light.ch1", out modelCorner2.light.ch1, out fail); if (fail) goto SET_FAIL;
            setDefault("modelCorner2.light.ch2", out modelCorner2.light.ch2, out fail); if (fail) goto SET_FAIL;

            setDefault("modelCorner3.isCreate", out modelCorner3.isCreate, out fail); if (fail) goto SET_FAIL;
            setDefault("modelCorner3.ID", out modelCorner3.ID, out fail); if (fail) goto SET_FAIL;
            setDefault("modelCorner3.algorism", out modelCorner3.algorism, out fail); if (fail) goto SET_FAIL;
            setDefault("modelCorner3.passScore", out modelCorner3.passScore, out fail); if (fail) goto SET_FAIL;
            setDefault("modelCorner3.angleStart", out modelCorner3.angleStart, out fail); if (fail) goto SET_FAIL;
            setDefault("modelCorner3.angleExtent", out modelCorner3.angleExtent, out fail); if (fail) goto SET_FAIL;
            setDefault("modelCorner3.exposureTime", out modelCorner3.exposureTime, out fail); if (fail) goto SET_FAIL;
            setDefault("modelCorner3.light.ch1", out modelCorner3.light.ch1, out fail); if (fail) goto SET_FAIL;
            setDefault("modelCorner3.light.ch2", out modelCorner3.light.ch2, out fail); if (fail) goto SET_FAIL;

            setDefault("modelCorner4.isCreate", out modelCorner4.isCreate, out fail); if (fail) goto SET_FAIL;
            setDefault("modelCorner4.ID", out modelCorner4.ID, out fail); if (fail) goto SET_FAIL;
            setDefault("modelCorner4.algorism", out modelCorner4.algorism, out fail); if (fail) goto SET_FAIL;
            setDefault("modelCorner4.passScore", out modelCorner4.passScore, out fail); if (fail) goto SET_FAIL;
            setDefault("modelCorner4.angleStart", out modelCorner4.angleStart, out fail); if (fail) goto SET_FAIL;
            setDefault("modelCorner4.angleExtent", out modelCorner4.angleExtent, out fail); if (fail) goto SET_FAIL;
            setDefault("modelCorner4.exposureTime", out modelCorner4.exposureTime, out fail); if (fail) goto SET_FAIL;
            setDefault("modelCorner4.light.ch1", out modelCorner4.light.ch1, out fail); if (fail) goto SET_FAIL;
            setDefault("modelCorner4.light.ch2", out modelCorner4.light.ch2, out fail); if (fail) goto SET_FAIL;
		...
		void setDefault(HTuple name, out para_member p, out bool fail)
		{
			try
			{
                ...
                id++; if (name == "modelCorner1.isCreate") setDefault(out p, name, id, 0, 0, 1, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelCorner1.ID") setDefault(out p, name, id, 0, 0, 1000, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelCorner1.algorism") setDefault(out p, name, id, 0, 0, 1, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelCorner1.passScore") setDefault(out p, name, id, 70, 30, 90, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelCorner1.angleStart") setDefault(out p, name, id, -30, -360, 0, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelCorner1.angleExtent") setDefault(out p, name, id, 60, 0, 360, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelCorner1.exposureTime") setDefault(out p, name, id, 5000, 1000, 30000, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelCorner1.light.ch1") setDefault(out p, name, id, 0, 0, 255, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelCorner1.light.ch2") setDefault(out p, name, id, 0, 0, 255, AUTHORITY.MAINTENCE.ToString(), "description");

                id++; if (name == "modelCorner2.isCreate") setDefault(out p, name, id, 0, 0, 1, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelCorner2.ID") setDefault(out p, name, id, 0, 0, 1000, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelCorner2.algorism") setDefault(out p, name, id, 0, 0, 1, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelCorner2.passScore") setDefault(out p, name, id, 70, 30, 90, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelCorner2.angleStart") setDefault(out p, name, id, -30, -360, 0, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelCorner2.angleExtent") setDefault(out p, name, id, 60, 0, 360, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelCorner2.exposureTime") setDefault(out p, name, id, 5000, 1000, 30000, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelCorner2.light.ch1") setDefault(out p, name, id, 0, 0, 255, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelCorner2.light.ch2") setDefault(out p, name, id, 0, 0, 255, AUTHORITY.MAINTENCE.ToString(), "description");

                id++; if (name == "modelCorner3.isCreate") setDefault(out p, name, id, 0, 0, 1, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelCorner3.ID") setDefault(out p, name, id, 0, 0, 1000, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelCorner3.algorism") setDefault(out p, name, id, 0, 0, 1, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelCorner3.passScore") setDefault(out p, name, id, 70, 30, 90, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelCorner3.angleStart") setDefault(out p, name, id, -30, -360, 0, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelCorner3.angleExtent") setDefault(out p, name, id, 60, 0, 360, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelCorner3.exposureTime") setDefault(out p, name, id, 5000, 1000, 30000, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelCorner3.light.ch1") setDefault(out p, name, id, 0, 0, 255, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelCorner3.light.ch2") setDefault(out p, name, id, 0, 0, 255, AUTHORITY.MAINTENCE.ToString(), "description");

                id++; if (name == "modelCorner4.isCreate") setDefault(out p, name, id, 0, 0, 1, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelCorner4.ID") setDefault(out p, name, id, 0, 0, 1000, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelCorner4.algorism") setDefault(out p, name, id, 0, 0, 1, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelCorner4.passScore") setDefault(out p, name, id, 70, 30, 90, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelCorner4.angleStart") setDefault(out p, name, id, -30, -360, 0, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelCorner4.angleExtent") setDefault(out p, name, id, 60, 0, 360, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelCorner4.exposureTime") setDefault(out p, name, id, 5000, 1000, 30000, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelCorner4.light.ch1") setDefault(out p, name, id, 0, 0, 255, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "modelCorner4.light.ch2") setDefault(out p, name, id, 0, 0, 255, AUTHORITY.MAINTENCE.ToString(), "description");
                ...
                id++; if (name == "detectDirection") setDefault(out p, name, id, 0, 1, 1, AUTHORITY.MAINTENCE.ToString(), "description");
	...
	public class DiagnosisParameter
	{
        ...
        public para_member inputTmsPCName;
        public para_member outputTmsPCName;
		...
		public void write(out bool r, string savepath = "C:\\PROTEC\\Data")
		{
			try
			{
                ...
                writeTuple(inputTmsPCName, i, out i);
                writeTuple(outputTmsPCName, i, out i);
		...
		public void upData(out bool r)
		{
			try
			{
                ...
                readTuple("inputTmsPCName", out inputTmsPCName, out fail); if (fail) goto SET_FAIL;
                readTuple("outputTmsPCName", out outputTmsPCName, out fail); if (fail) goto SET_FAIL;
		...
		void setDefault(out bool r)
		{
            ...
            setDefault("inputTmsPCName", out inputTmsPCName, out fail); if (fail) goto SET_FAIL;
            setDefault("outputTmsPCName", out outputTmsPCName, out fail); if (fail) goto SET_FAIL;
		...
		void setDefault(HTuple name, out para_member p, out bool fail)
		{
			try
			{
                ...
                id++; if (name == "inputTmsPCName") setDefault(out p, name, id, 0, 0, 255, AUTHORITY.MAINTENCE.ToString(), "127.0.0.1"/*"protec-277207f4"*/);
                id++; if (name == "outputTmsPCName") setDefault(out p, name, id, 0, 0, 255, AUTHORITY.MAINTENCE.ToString(), "127.0.0.1"/*"protec-277207f4"*/);
    ...
    #region Unloader
    public class UnloaderParameter
    {
        RetValue ret;
        public UnitCode unitCode;
        public para_member slotCount = new para_member();
        public para_member slotPitch = new para_member();
        public para_member PusherPos = new para_member();
        public para_member MagazineInCheck = new para_member();
        public para_member AreaCheck = new para_member();
        public parameterZ[,] MagazinePos = new parameterZ[(int)MG_NUM.MAX, (int)UnitCodeSlotSelect.MAX];
        public para_member ReadyPos = new para_member();
        ...
        public void write(out bool r, string savepath = "C:\\PROTEC\\Data")
        {
            try
            {
                ...
                writeTuple(slotCount, i, out i);
                writeTuple(slotPitch, i, out i);
                writeTuple(PusherPos, i, out i);
                writeTuple(MagazineInCheck, i, out i);
                writeTuple(AreaCheck, i, out i);
                writeTuple(ReadyPos, i, out i);

                for (int ix = 0; ix < (int)MG_NUM.MAX; ix++)
                {
                    for (int iy = 0; iy < (int)UnitCodeSlotSelect.MAX; iy++)
                        writeTuple(MagazinePos[ix, iy].z, i, out i);
                }
        ...
        public void upData(out bool r)
        {
            ...
            {
                bool fail;
                readTuple("slotCount", out slotCount, out fail); if (fail) goto SET_FAIL;
                readTuple("slotPitch", out slotPitch, out fail); if (fail) goto SET_FAIL;
                readTuple("PusherPos", out PusherPos, out fail); if (fail) goto SET_FAIL;

                readTuple("MagazineInCheck", out MagazineInCheck, out fail); if (fail) goto SET_FAIL;
                readTuple("AreaCheck", out AreaCheck, out fail); if (fail) goto SET_FAIL;
                readTuple("ReadyPos", out ReadyPos, out fail); if (fail) goto SET_FAIL;

                readTuple("MagazinePos[0,0]", out MagazinePos[0, 0].z, out fail); if (fail) goto SET_FAIL;
                readTuple("MagazinePos[0,1]", out MagazinePos[0, 1].z, out fail); if (fail) goto SET_FAIL;

                readTuple("MagazinePos[1,0]", out MagazinePos[1, 0].z, out fail); if (fail) goto SET_FAIL;
                readTuple("MagazinePos[1,1]", out MagazinePos[1, 1].z, out fail); if (fail) goto SET_FAIL;

                readTuple("MagazinePos[2,0]", out MagazinePos[2, 0].z, out fail); if (fail) goto SET_FAIL;
                readTuple("MagazinePos[2,1]", out MagazinePos[2, 1].z, out fail); if (fail) goto SET_FAIL;
        ...
        void setDefault(out bool r)
        {
            ...
            bool fail;
            setDefault("slotCount", out slotCount, out fail); if (fail) goto SET_FAIL;
            setDefault("slotPitch", out slotPitch, out fail); if (fail) goto SET_FAIL;
            setDefault("PusherPos", out PusherPos, out fail); if (fail) goto SET_FAIL;

            setDefault("MagazineInCheck", out MagazineInCheck, out fail); if (fail) goto SET_FAIL;
            setDefault("AreaCheck", out AreaCheck, out fail); if (fail) goto SET_FAIL;
            setDefault("ReadyPos", out ReadyPos, out fail); if (fail) goto SET_FAIL;

            setDefault("MagazinePos[0,0]", out MagazinePos[0, 0].z, out fail); if (fail) goto SET_FAIL;
            setDefault("MagazinePos[0,1]", out MagazinePos[0, 1].z, out fail); if (fail) goto SET_FAIL;

            setDefault("MagazinePos[1,0]", out MagazinePos[1, 0].z, out fail); if (fail) goto SET_FAIL;
            setDefault("MagazinePos[1,1]", out MagazinePos[1, 1].z, out fail); if (fail) goto SET_FAIL;

            setDefault("MagazinePos[2,0]", out MagazinePos[2, 0].z, out fail); if (fail) goto SET_FAIL;
            setDefault("MagazinePos[2,1]", out MagazinePos[2, 1].z, out fail); if (fail) goto SET_FAIL;
        ...
        void setDefault(HTuple name, out para_member p, out bool fail)
        {
            try
            {
                p = new para_member(); p.id = -1;
                int id = -1;
                id++; if (name == "slotCount") setDefault(out p, name, id, 10, 1, 30, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "slotPitch") setDefault(out p, name, id, 10, 0.5, 30, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "PusherPos") setDefault(out p, name, id, 0, -100000, 100000, AUTHORITY.MAINTENCE.ToString(), "description");

                id++; if (name == "MagazineInCheck") setDefault(out p, name, id, 1, 0, 1, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "AreaCheck") setDefault(out p, name, id, 1, 0, 1, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "ReadyPos") setDefault(out p, name, id, 0, -900000, 900000, AUTHORITY.MAINTENCE.ToString(), "description");
                
                for (int i = 0; i < (int)MG_NUM.MAX; i++)
                {
                    for (int m = 0; m < (int)UnitCodeSlotSelect.MAX; m++)
                    {
                        id++; if (name == "MagazinePos[" + i.ToString() + "," + m.ToString() + "]") setDefault(out p, name, id, 0, -30000, 30000, AUTHORITY.MAINTENCE.ToString(), "description");
                    }
                }
...
// 		public para_member placeTimeForceCheckLimit; 
        ...
        public para_member useHeadMode;
        public para_member unloaderControl;
        public para_member useAutoTiltCheck;
...
// 				writeTuple(placeTimeForceCheckLimit, i, out i);
                ...
                writeTuple(useHeadMode, i, out i);
                writeTuple(unloaderControl, i, out i);
                writeTuple(useAutoTiltCheck, i, out i);
...
// 				readTuple("placeTimeForceCheckLimit", out placeTimeForceCheckLimit, out fail); if (fail) goto SET_FAIL;
                ...
                readTuple("useHeadMode", out useHeadMode, out fail); if (fail) goto SET_FAIL;
                readTuple("unloaderControl", out unloaderControl, out fail); if (fail) goto SET_FAIL;
                readTuple("useAutoTiltCheck", out useAutoTiltCheck, out fail); if (fail) goto SET_FAIL;
...
// 			setDefault("placeTimeForceCheckLimit", out placeTimeForceCheckLimit, out fail); if (fail) goto SET_FAIL;
            ...
            setDefault("useHeadMode", out useHeadMode, out fail); if (fail) goto SET_FAIL;
            setDefault("unloaderControl", out unloaderControl, out fail); if (fail) goto SET_FAIL;
            setDefault("useAutoTiltCheck", out useAutoTiltCheck, out fail); if (fail) goto SET_FAIL;
...
// 				id++; if (name == "placeTimeForceCheckLimit") setDefault(out p, name, id, 0.2, 0, 1, AUTHORITY.MAINTENCE.ToString(), "Place Time Force Check Limit[kg]");
                ...
                id++; if (name == "useHeadMode") setDefault(out p, name, id, 2, 0, 2, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "unloaderControl") setDefault(out p, name, id, 1, 0, 1, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "useAutoTiltCheck") setDefault(out p, name, id, 0, 0, 1, AUTHORITY.MAINTENCE.ToString(), "description");
    ...
    #region Advanced information
    public class AdvancedParameter
    {
        RetValue ret;
        public UnitCode unitCode;
        public para_member LightCOMPort;
        public para_member motionDelay;
        public para_member retryCount;
        public para_member useContinousErrorLimit;
        public para_member continousErrorLimit;
        public para_member useTotalErrorLimit;
        public para_member totalErrorLimit;
        public para_member preVisionErrSkip;		// Pre 로부터 Vision Skip, Error 등을 검사항목에서 제외함.
        public para_member CheckDoorLock;
        public para_member passwordProtect;
        // 		public para_member TMSUsage;
        public para_member MCCLOGUsage;
        public para_member reworkMsgCount;
        public para_member reworkMsgUsage;
        public para_member autoRetry;
        public para_member autoRetryCount;
        public para_member useErrorSkip;
        public para_member errorSkipLimit;
        //---------------------------------------------

        public para_member Init_LightCOMPort;
        public para_member Init_CheckDoorLock;
        public para_member Init_motionDelay;
        public para_member Init_retryCount;
        public para_member Init_useContinousErrorLimit;
        public para_member Init_continousErrorLimit;
        public para_member Init_useTotalErrorLimit;
        public para_member Init_totalErrorLimit;
        public para_member Init_useErrorSkip;
        public para_member Init_errorSkipLimit;
        public para_member Init_reworkMsgUsage;
        public para_member Init_reworkMsgCount;
        public para_member Init_preVisionErrSkip;
        public para_member Init_passwordProtect;
        public para_member Init_MCCLOGUsage;
        public para_member Init_autoRetry;
        public para_member Init_autoRetryCount;
        public para_member Init_unloaderControl;

        public HTuple saveTuple;
        public void write(out bool r)
        {
            try
            {
                int i = 0;
                saveTuple = new HTuple();
                writeTuple(LightCOMPort, i, out i);
                writeTuple(motionDelay, i, out i);
                writeTuple(retryCount, i, out i);
                writeTuple(useContinousErrorLimit, i, out i);
                writeTuple(continousErrorLimit, i, out i);
                writeTuple(useTotalErrorLimit, i, out i);
                writeTuple(totalErrorLimit, i, out i);
                writeTuple(preVisionErrSkip, i, out i);
                writeTuple(CheckDoorLock, i, out i);
                writeTuple(passwordProtect, i, out i);
                // 				writeTuple(TMSUsage, i, out i);
                writeTuple(MCCLOGUsage, i, out i);
                writeTuple(reworkMsgUsage, i, out i);
                writeTuple(reworkMsgCount, i, out i);
                writeTuple(autoRetry, i, out i);
                writeTuple(autoRetryCount, i, out i);
                writeTuple(useErrorSkip, i, out i);
                writeTuple(errorSkipLimit, i, out i);
        ...
        public void upData(out bool r)
        {
            ...
            {
                bool fail;
                readTuple("LightCOMPort", out LightCOMPort, out fail); if (fail) goto SET_FAIL;
                readTuple("motionDelay", out motionDelay, out fail); if (fail) goto SET_FAIL;
                readTuple("VisionRetryCount", out retryCount, out fail); if (fail) goto SET_FAIL;
                readTuple("UseContinousErrorLimit", out useContinousErrorLimit, out fail); if (fail) goto SET_FAIL;
                readTuple("ContinousErrorLimit", out continousErrorLimit, out fail); if (fail) goto SET_FAIL;
                readTuple("UseTotalErrorLimit", out useTotalErrorLimit, out fail); if (fail) goto SET_FAIL;
                readTuple("TotalErrorLimit", out totalErrorLimit, out fail); if (fail) goto SET_FAIL;
                readTuple("PreVisionErrorSkip", out preVisionErrSkip, out fail); if (fail) goto SET_FAIL;
                readTuple("CheckDoorLock", out CheckDoorLock, out fail); if (fail) goto SET_FAIL;
                readTuple("passwordProtect", out passwordProtect, out fail); if (fail) goto SET_FAIL;
                readTuple("MCCLOGUsage", out MCCLOGUsage, out fail); if (fail) goto SET_FAIL;
                readTuple("ReworkMsgUsage", out reworkMsgUsage, out fail); if (fail) goto SET_FAIL;
                readTuple("ReworkMsgCount", out reworkMsgCount, out fail); if (fail) goto SET_FAIL;
                readTuple("MCCLOGUsage", out MCCLOGUsage, out fail); if (fail) goto SET_FAIL;
                readTuple("autoRetry", out autoRetry, out fail); if (fail) goto SET_FAIL;
                readTuple("autoRetryCount", out autoRetryCount, out fail); if (fail) goto SET_FAIL;
                readTuple("useErrorSkip", out useErrorSkip, out fail); if (fail) goto SET_FAIL;
                readTuple("errorSkipLimit", out errorSkipLimit, out fail); if (fail) goto SET_FAIL;
        ...
        {
            FormParaList ff = new FormParaList();
            ff.activate(mc.para.ADVCD.saveTuple);
            ff.ShowDialog();
            saveTuple = ff.saveTuple;
            upData(out ret.b);
            write(out ret.b);
            mc.commMPC.writeFDCFile();
        }
        ...
        void setDefault(out bool r)
        {
            ...
            bool fail;
            setDefault("LightCOMPort", out LightCOMPort, out fail); if (fail) goto SET_FAIL;
            setDefault("motionDelay", out motionDelay, out fail); if (fail) goto SET_FAIL;
            setDefault("VisionRetryCount", out retryCount, out fail); if (fail) goto SET_FAIL;
            setDefault("UseContinousErrorLimit", out useContinousErrorLimit, out fail); if (fail) goto SET_FAIL;
            setDefault("ContinousErrorLimit", out continousErrorLimit, out fail); if (fail) goto SET_FAIL;
            setDefault("UseTotalErrorLimit", out useTotalErrorLimit, out fail); if (fail) goto SET_FAIL;
            setDefault("TotalErrorLimit", out totalErrorLimit, out fail); if (fail) goto SET_FAIL;
            setDefault("PreVisionErrorSkip", out preVisionErrSkip, out fail); if (fail) goto SET_FAIL;
            setDefault("CheckDoorLock", out CheckDoorLock, out fail); if (fail) goto SET_FAIL;
            setDefault("passwordProtect", out passwordProtect, out fail); if (fail) goto SET_FAIL;
            setDefault("MCCLOGUsage", out MCCLOGUsage, out fail); if (fail) goto SET_FAIL;
            setDefault("ReworkMsgUsage", out reworkMsgUsage, out fail); if (fail) goto SET_FAIL;
            setDefault("ReworkMsgCount", out reworkMsgCount, out fail); if (fail) goto SET_FAIL;
            setDefault("autoRetry", out autoRetry, out fail); if (fail) goto SET_FAIL;
            setDefault("autoRetryCount", out autoRetryCount, out fail); if (fail) goto SET_FAIL;
            setDefault("useErrorSkip", out useErrorSkip, out fail); if (fail) goto SET_FAIL;
            setDefault("errorSkipLimit", out errorSkipLimit, out fail); if (fail) goto SET_FAIL;
        ...
        void setDefault(HTuple name, out para_member p, out bool fail)
        {
            try
            {
                p = new para_member(); p.id = -1;
                int id = -1;
                id++; if (name == "LightCOMPort") setDefault(out p, name, id, 1, 1, 30, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "motionDelay") setDefault(out p, name, id, 50, 0, 5000, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "VisionRetryCount") setDefault(out p, name, id, 3, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "UseContinousErrorLimit") setDefault(out p, name, id, 0, 0, 1, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "ContinousErrorLimit") setDefault(out p, name, id, 3, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "UseTotalErrorLimit") setDefault(out p, name, id, 0, 0, 1, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "TotalErrorLimit") setDefault(out p, name, id, 10, 0, 500, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "PreVisionErrorSkip") setDefault(out p, name, id, 0, 0, 1, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "CheckDoorLock") setDefault(out p, name, id, 0, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "passwordProtect") setDefault(out p, name, id, 0, 0, 1, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "MCCLOGUsage") setDefault(out p, name, id, 0, 0, 1, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "ReworkMsgUsage") setDefault(out p, name, id, 0, 0, 1, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "ReworkMsgCount") setDefault(out p, name, id, 3, 0, 100, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "autoRetry") setDefault(out p, name, id, 0, 0, 100, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "autoRetryCount") setDefault(out p, name, id, 3, 0, 100, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "useErrorSkip") setDefault(out p, name, id, 0, 0, 1, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "errorSkipLimit") setDefault(out p, name, id, 3, 0, 90, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "mcLanguage") setDefault(out p, name, id, 0, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "epoxyPositionCheckMode") setDefault(out p, name, id, 0, 0, 1, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "FOVmode") setDefault(out p, name, id, 0, 0, 1, AUTHORITY.MAINTENCE.ToString(), "description");
	...
	public class calibrationParameter
	{
		...
		public parameterXY[] HDC_TOOL = new parameterXY[2];
        public parameterXY[] Tool_Con = new parameterXY[2];
		public parameterXY HDC_LASER = new parameterXY();
		public parameterXYW HDC_PD = new parameterXYW();
        ...
        public parameterForce Tool_Force1 = new parameterForce();
        public parameterForce Tool_Force2 = new parameterForce();
        ...
        // 1123 . Place Offset 은 50, 20 인데 여기는 30, 20 으로 되어있네요..
        public para_member[,] PDHeightC1 = new para_member[50, 20];
        public para_member[,] PDHeightC2 = new para_member[50, 20];
        public para_member[,] PDHeightC3 = new para_member[50, 20];
        public para_member[,] PDHeightC4 = new para_member[50, 20];

        public para_member probeValue = new para_member();          // Touch Probe Z 높이를 구할 때 이 값을 저장해뒀다가, Height Offset 에서 써먹자
        public para_member probePosZ = new para_member();          // Touch Probe Z 높이를 구할 때 이 값을 저장해뒀다가, Height Offset 에서 써먹자
		...
		public void write(out bool r, string savepath = "C:\\PROTEC\\Data")
		{
			try
			{
                ...
                {
                    writeTuple(HDC_TOOL[ix].x, i, out i);
                    writeTuple(HDC_TOOL[ix].y, i, out i);
                }

                for (int ix = 0; ix < 2; ix++)
                {
                    writeTuple(Tool_Con[ix].x, i, out i);
                    writeTuple(Tool_Con[ix].y, i, out i);
                }

				writeTuple(HDC_LASER.x, i, out i); writeTuple(HDC_LASER.y, i, out i);
				writeTuple(HDC_PD.x, i, out i); writeTuple(HDC_PD.y, i, out i); writeTuple(HDC_PD.w, i, out i);
                ...
                for (int ix = 0; ix < 30; ix++)
                {
                    for (int iy = 0; iy < 20; iy++)
                    {
                        writeTuple(PDHeightC1[ix, iy], i, out i);
                        writeTuple(PDHeightC2[ix, iy], i, out i);
                        writeTuple(PDHeightC3[ix, iy], i, out i);
                        writeTuple(PDHeightC4[ix, iy], i, out i);
                    }
                }
                writeTuple(ulc.x, i, out i);
                writeTuple(ulc.y, i, out i);

				writeTuple(conveyorEdge.x, i, out i); writeTuple(conveyorEdge.y, i, out i);
				
                for(int ix = 0; ix < 2; ix++) writeTuple(toolAngleOffset[ix], i, out i);
                ...
                for (int j = 0; j < 20; j++)
                {
                    writeTuple(Tool_Force1.forceLevel[j], i, out i);
                    writeTuple(Tool_Force1.bottomForce[j], i, out i);
                    writeTuple(Tool_Force1.topForce[j], i, out i);
                    writeTuple(Tool_Force1.heightLevel[j], i, out i);
                }

                for (int j = 0; j < 20; j++)
                {
                    writeTuple(Tool_Force2.forceLevel[j], i, out i);
                    writeTuple(Tool_Force2.bottomForce[j], i, out i);
                    writeTuple(Tool_Force2.topForce[j], i, out i);
                    writeTuple(Tool_Force2.heightLevel[j], i, out i);
                }
                ...
                writeTuple(probeValue, i, out i);
                writeTuple(probePosZ, i, out i);
		...
		public void upData(out bool r)
		{
			try
			{
                ...
                for (int i = 0; i < 2; i++)
                {
                    readTuple("HDC_TOOL[" + i.ToString() + "].x", out HDC_TOOL[i].x, out fail); if (fail) goto SET_FAIL;
                    readTuple("HDC_TOOL[" + i.ToString() + "].y", out HDC_TOOL[i].y, out fail); if (fail) goto SET_FAIL;
                }

                readTuple("Tool_Con[0].x", out Tool_Con[0].x, out fail); if (fail) goto SET_FAIL;
                readTuple("Tool_Con[0].y", out Tool_Con[0].y, out fail); if (fail) goto SET_FAIL;

                readTuple("Tool_Con[1].x", out Tool_Con[1].x, out fail); if (fail) goto SET_FAIL;
                readTuple("Tool_Con[1].y", out Tool_Con[1].y, out fail); if (fail) goto SET_FAIL;
				...
				readTuple("HDC_PD.x", out HDC_PD.x, out fail); if (fail) goto SET_FAIL;
				readTuple("HDC_PD.y", out HDC_PD.y, out fail); if (fail) goto SET_FAIL;
                readTuple("HDC_PD.w", out HDC_PD.w, out fail); if (fail) goto SET_FAIL;
                ...
                for (int ix = 0; ix < 30; ix++)
                {
                    for (int iy = 0; iy < 20; iy++)
                    {
                        readTuple("PDHeightC1[" + ix.ToString() + "," + iy.ToString() + "]", out PDHeightC1[ix, iy], out fail); if (fail) goto SET_FAIL;
                        readTuple("PDHeightC2[" + ix.ToString() + "," + iy.ToString() + "]", out PDHeightC2[ix, iy], out fail); if (fail) goto SET_FAIL;
                        readTuple("PDHeightC3[" + ix.ToString() + "," + iy.ToString() + "]", out PDHeightC3[ix, iy], out fail); if (fail) goto SET_FAIL;
                        readTuple("PDHeightC4[" + ix.ToString() + "," + iy.ToString() + "]", out PDHeightC4[ix, iy], out fail); if (fail) goto SET_FAIL;
                    }
                }
                ...
                for (int ix = 0; ix < 2; ix++ ) readTuple("toolAngleOffset[" + ix.ToString() + "]", out toolAngleOffset[ix], out fail); if (fail) goto SET_FAIL;
                ...
                readTuple("z.loadCellPos", out z.loadCellPos, out fail); if (fail) goto SET_FAIL;
                ...
                for (int i = 0; i < 2; i++) readTuple("z.heightOffset[" + i.ToString() + "]", out z.heightOffset[i], out fail); if (fail) goto SET_FAIL;
                ...
                for(int j = 0; j < 20; j++)
                {
                    readTuple("Tool_Force1.forceLevel[" + j.ToString() + "]", out Tool_Force1.forceLevel[j], out fail); if (fail) goto SET_FAIL;
                    readTuple("Tool_Force1.bottomForce[" + j.ToString() + "]", out Tool_Force1.bottomForce[j], out fail); if (fail) goto SET_FAIL;
                    readTuple("Tool_Force1.topForce[" + j.ToString() + "]", out Tool_Force1.topForce[j], out fail); if (fail) goto SET_FAIL;
                    readTuple("Tool_Force1.heightLevel[" + j.ToString() + "]", out Tool_Force1.heightLevel[j], out fail); if (fail) goto SET_FAIL;
                }

                for (int j = 0; j < 20; j++)
                {
                    readTuple("Tool_Force2.forceLevel[" + j.ToString() + "]", out Tool_Force2.forceLevel[j], out fail); if (fail) goto SET_FAIL;
                    readTuple("Tool_Force2.bottomForce[" + j.ToString() + "]", out Tool_Force2.bottomForce[j], out fail); if (fail) goto SET_FAIL;
                    readTuple("Tool_Force2.topForce[" + j.ToString() + "]", out Tool_Force2.topForce[j], out fail); if (fail) goto SET_FAIL;
                    readTuple("Tool_Force2.heightLevel[" + j.ToString() + "]", out Tool_Force2.heightLevel[j], out fail); if (fail) goto SET_FAIL;
                }
                ...
                readTuple("probeValue", out probeValue, out fail); if (fail) goto SET_FAIL;
                readTuple("probePosZ", out probePosZ, out fail); if (fail) goto SET_FAIL;
		...
		void setDefault(out bool r)
		{
            ...
            for (int i = 0; i < 2; i++ )
            {
                setDefault("HDC_TOOL[" + i.ToString() + "].x", out HDC_TOOL[i].x, out fail); if (fail) goto SET_FAIL;
                setDefault("HDC_TOOL[" + i.ToString() + "].y", out HDC_TOOL[i].y, out fail); if (fail) goto SET_FAIL;
            }

            setDefault("Tool_Con[0].x", out Tool_Con[0].x, out fail); if (fail) goto SET_FAIL;
            setDefault("Tool_Con[0].y", out Tool_Con[0].y, out fail); if (fail) goto SET_FAIL;
            setDefault("Tool_Con[1].x", out Tool_Con[1].x, out fail); if (fail) goto SET_FAIL;
            setDefault("Tool_Con[1].y", out Tool_Con[1].y, out fail); if (fail) goto SET_FAIL;
			...
			setDefault("HDC_PD.x", out HDC_PD.x, out fail); if (fail) goto SET_FAIL;
			setDefault("HDC_PD.y", out HDC_PD.y, out fail); if (fail) goto SET_FAIL;
            setDefault("HDC_PD.y", out HDC_PD.w, out fail); if (fail) goto SET_FAIL;
            ...
            for (int ix = 0; ix < 30; ix++)
            {
                for (int iy = 0; iy < 20; iy++)
                {
                    setDefault("PDHeightC1[" + ix.ToString() + "," + iy.ToString() + "]", out PDHeightC1[ix, iy], out fail); if (fail) goto SET_FAIL;
                    setDefault("PDHeightC2[" + ix.ToString() + "," + iy.ToString() + "]", out PDHeightC2[ix, iy], out fail); if (fail) goto SET_FAIL;
                    setDefault("PDHeightC3[" + ix.ToString() + "," + iy.ToString() + "]", out PDHeightC3[ix, iy], out fail); if (fail) goto SET_FAIL;
                    setDefault("PDHeightC4[" + ix.ToString() + "," + iy.ToString() + "]", out PDHeightC4[ix, iy], out fail); if (fail) goto SET_FAIL;
                }
            }
            ...
            for(int ix = 0; ix < 2; ix++) setDefault("toolAngleOffset[" + ix.ToString() + "]", out toolAngleOffset[ix], out fail); if (fail) goto SET_FAIL;
            ...
            setDefault("z.loadCellPos", out z.loadCellPos, out fail); if (fail) goto SET_FAIL;
            ...
            for (int i = 0; i < 2; i++) setDefault("z.heightOffset[" + i.ToString() + "]", out z.heightOffset[i], out fail);
            ...
            for (int i = 0; i < 20; i++)
            {
                setDefault("Tool_Force1.forceLevel[" + i.ToString() + "]", out Tool_Force1.forceLevel[i], out fail); if (fail) goto SET_FAIL;
                setDefault("Tool_Force1.bottomForce[" + i.ToString() + "]", out Tool_Force1.bottomForce[i], out fail); if (fail) goto SET_FAIL;
                setDefault("Tool_Force1.topForce[" + i.ToString() + "]", out Tool_Force1.topForce[i], out fail); if (fail) goto SET_FAIL;
                setDefault("Tool_Force1.heightLevel[" + i.ToString() + "]", out Tool_Force1.heightLevel[i], out fail); if (fail) goto SET_FAIL;
            }

            for (int i = 0; i < 20; i++)
            {
                setDefault("Tool_Force2.forceLevel[" + i.ToString() + "]", out Tool_Force2.forceLevel[i], out fail); if (fail) goto SET_FAIL;
                setDefault("Tool_Force2.bottomForce[" + i.ToString() + "]", out Tool_Force2.bottomForce[i], out fail); if (fail) goto SET_FAIL;
                setDefault("Tool_Force2.topForce[" + i.ToString() + "]", out Tool_Force2.topForce[i], out fail); if (fail) goto SET_FAIL;
                setDefault("Tool_Force2.heightLevel[" + i.ToString() + "]", out Tool_Force2.heightLevel[i], out fail); if (fail) goto SET_FAIL;
            }
            ...
            setDefault("probeValue", out probeValue, out fail); if (fail) goto SET_FAIL;
            setDefault("probePosZ", out probePosZ, out fail); if (fail) goto SET_FAIL;
		...
		void setDefault(HTuple name, out para_member p, out bool fail)
		{
			try
			{
                ...
                for (int i = 0; i < 2; i++ )
                {
                    id++; if (name == "HDC_TOOL[" + i.ToString() + "].x") setDefault(out p, name, id, 0, lowLimitXY, highLimitXY, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "HDC_TOOL[" + i.ToString() + "].y") setDefault(out p, name, id, 0, lowLimitXY, highLimitXY, AUTHORITY.MAINTENCE.ToString(), "description");
                }

                id++; if (name == "Tool_Con[0].x") setDefault(out p, name, id, 0, lowLimitXY, highLimitXY, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "Tool_Con[0].y") setDefault(out p, name, id, 0, lowLimitXY, highLimitXY, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "Tool_Con[1].x") setDefault(out p, name, id, 0, lowLimitXY, highLimitXY, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "Tool_Con[1].y") setDefault(out p, name, id, 0, lowLimitXY, highLimitXY, AUTHORITY.MAINTENCE.ToString(), "description");
                ...
                id++; if (name == "HDC_PD.x") setDefault(out p, name, id, 0, -50000000, 50000000, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "HDC_PD.y") setDefault(out p, name, id, 0, -50000000, 50000000, AUTHORITY.MAINTENCE.ToString(), "description");
                id++; if (name == "HDC_PD.w") setDefault(out p, name, id, 0, -50000000, 50000000, AUTHORITY.MAINTENCE.ToString(), "description");
                ...
                for (int ix = 0; ix < 30; ix++)
                {
                    for (int iy = 0; iy < 20; iy++)
                    {
                        id++; if (name == "PDHeightC1[" + ix.ToString() + "," + iy.ToString() + "]") setDefault(out p, name, id, 0, -1000, 1000, AUTHORITY.MAINTENCE.ToString(), "Pedestal Height C1");
                        id++; if (name == "PDHeightC2[" + ix.ToString() + "," + iy.ToString() + "]") setDefault(out p, name, id, 0, -1000, 1000, AUTHORITY.MAINTENCE.ToString(), "Pedestal Height C2");
                        id++; if (name == "PDHeightC3[" + ix.ToString() + "," + iy.ToString() + "]") setDefault(out p, name, id, 0, -1000, 1000, AUTHORITY.MAINTENCE.ToString(), "Pedestal Height C3");
                        id++; if (name == "PDHeightC4[" + ix.ToString() + "," + iy.ToString() + "]") setDefault(out p, name, id, 0, -1000, 1000, AUTHORITY.MAINTENCE.ToString(), "Pedestal Height C4");
                    }
                }
                ...
                for (int ix = 0; ix < 2; ix++)
                {
                    id++; if (name == "toolAngleOffset[" + ix.ToString() + "]") setDefault(out p, name, id, 0, -18000, 180000, AUTHORITY.MAINTENCE.ToString(), "description");
                }
                ...
                id++; if (name == "z.loadCellPos") setDefault(out p, name, id, 0, -50000, 50000, AUTHORITY.MAINTENCE.ToString(), "description");
                ...
                for (int i = 0; i < 2; i++)
                {
                    id++; if (name == "z.heightOffset[" + i.ToString() + "]") setDefault(out p, name, id, 0, -10000, 10000, AUTHORITY.MAINTENCE.ToString(), "description");
                }
                ...
                for (int i = 0; i < 2; i++ )
                {
                    id++; if (name == "force[" + i.ToString() + "].A[0]") setDefault(out p, name, id, 1, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].B[0]") setDefault(out p, name, id, 0, 0, 50, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].C[0]") setDefault(out p, name, id, 1, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].D[0]") setDefault(out p, name, id, 1, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].A[1]") setDefault(out p, name, id, 2, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].B[1]") setDefault(out p, name, id, 0, 0, 50, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].C[1]") setDefault(out p, name, id, 2, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].D[1]") setDefault(out p, name, id, 2, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].A[2]") setDefault(out p, name, id, 3, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].B[2]") setDefault(out p, name, id, 0, 0, 50, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].C[2]") setDefault(out p, name, id, 3, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].D[2]") setDefault(out p, name, id, 3, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].A[3]") setDefault(out p, name, id, 4, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].B[3]") setDefault(out p, name, id, 0, 0, 50, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].C[3]") setDefault(out p, name, id, 4, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].D[3]") setDefault(out p, name, id, 4, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].A[4]") setDefault(out p, name, id, 5, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].B[4]") setDefault(out p, name, id, 0, 0, 50, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].C[4]") setDefault(out p, name, id, 5, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].D[4]") setDefault(out p, name, id, 5, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].A[5]") setDefault(out p, name, id, 6, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].B[5]") setDefault(out p, name, id, 0, 0, 50, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].C[5]") setDefault(out p, name, id, 6, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].D[5]") setDefault(out p, name, id, 6, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].A[6]") setDefault(out p, name, id, 7, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].B[6]") setDefault(out p, name, id, 0, 0, 50, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].C[6]") setDefault(out p, name, id, 7, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].D[6]") setDefault(out p, name, id, 7, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].A[7]") setDefault(out p, name, id, 8, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].B[7]") setDefault(out p, name, id, 0, 0, 50, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].C[7]") setDefault(out p, name, id, 8, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].D[7]") setDefault(out p, name, id, 8, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].A[8]") setDefault(out p, name, id, 9, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].B[8]") setDefault(out p, name, id, 0, 0, 50, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].C[8]") setDefault(out p, name, id, 9, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].D[8]") setDefault(out p, name, id, 9, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].A[9]") setDefault(out p, name, id, 10, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].B[9]") setDefault(out p, name, id, 0, 0, 50, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].C[9]") setDefault(out p, name, id, 10, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].D[9]") setDefault(out p, name, id, 10, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].A[10]") setDefault(out p, name, id, 1, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].B[10]") setDefault(out p, name, id, 0, 0, 50, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].C[10]") setDefault(out p, name, id, 1, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].D[10]") setDefault(out p, name, id, 1, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].A[11]") setDefault(out p, name, id, 2, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].B[11]") setDefault(out p, name, id, 0, 0, 50, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].C[11]") setDefault(out p, name, id, 2, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].D[11]") setDefault(out p, name, id, 2, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].A[12]") setDefault(out p, name, id, 3, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].B[12]") setDefault(out p, name, id, 0, 0, 50, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].C[12]") setDefault(out p, name, id, 3, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].D[12]") setDefault(out p, name, id, 3, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].A[13]") setDefault(out p, name, id, 4, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].B[13]") setDefault(out p, name, id, 0, 0, 50, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].C[13]") setDefault(out p, name, id, 4, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].D[13]") setDefault(out p, name, id, 4, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].A[14]") setDefault(out p, name, id, 5, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].B[14]") setDefault(out p, name, id, 0, 0, 50, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].C[14]") setDefault(out p, name, id, 5, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].D[14]") setDefault(out p, name, id, 5, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].A[15]") setDefault(out p, name, id, 6, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].B[15]") setDefault(out p, name, id, 0, 0, 50, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].C[15]") setDefault(out p, name, id, 6, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].D[15]") setDefault(out p, name, id, 6, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].A[16]") setDefault(out p, name, id, 7, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].B[16]") setDefault(out p, name, id, 0, 0, 50, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].C[16]") setDefault(out p, name, id, 7, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].D[16]") setDefault(out p, name, id, 7, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].A[17]") setDefault(out p, name, id, 8, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].B[17]") setDefault(out p, name, id, 0, 0, 50, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].C[17]") setDefault(out p, name, id, 8, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].D[17]") setDefault(out p, name, id, 8, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].A[18]") setDefault(out p, name, id, 9, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].B[18]") setDefault(out p, name, id, 0, 0, 50, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].C[18]") setDefault(out p, name, id, 9, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].D[18]") setDefault(out p, name, id, 9, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].A[19]") setDefault(out p, name, id, 10, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].B[19]") setDefault(out p, name, id, 0, 0, 50, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].C[19]") setDefault(out p, name, id, 10, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "force[" + i.ToString() + "].D[19]") setDefault(out p, name, id, 10, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                }
                id++; if (name == "force.touchOffset") setDefault(out p, name, id, 200, 100, 800, AUTHORITY.MAINTENCE.ToString(), "description");

                for (int i = 0; i < 20; i++)
                {
                    id++; if (name == "Tool_Force1.forceLevel[" + i.ToString() + "]") setDefault(out p, name, id, 10, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "Tool_Force1.bottomForce[" + i.ToString() + "]") setDefault(out p, name, id, 10, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "Tool_Force1.topForce[" + i.ToString() + "]") setDefault(out p, name, id, 10, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "Tool_Force1.heightLevel[" + i.ToString() + "]") setDefault(out p, name, id, 0, -100000, 0, AUTHORITY.MAINTENCE.ToString(), "description");
                }

                for (int i = 0; i < 20; i++)
                {
                    id++; if (name == "Tool_Force2.forceLevel[" + i.ToString() + "]") setDefault(out p, name, id, 10, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "Tool_Force2.bottomForce[" + i.ToString() + "]") setDefault(out p, name, id, 10, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "Tool_Force2.topForce[" + i.ToString() + "]") setDefault(out p, name, id, 10, 0, 10, AUTHORITY.MAINTENCE.ToString(), "description");
                    id++; if (name == "Tool_Force2.heightLevel[" + i.ToString() + "]") setDefault(out p, name, id, 0, -100000, 0, AUTHORITY.MAINTENCE.ToString(), "description");
                }
                ...
                id++; if (name == "probeValue") setDefault(out p, name, id, 0, 0, 10000, AUTHORITY.MAINTENCE.ToString(), "Touch Probe Value");
                id++; if (name == "probePosZ") setDefault(out p, name, id, 0, -50000, 50000, AUTHORITY.MAINTENCE.ToString(), "Touch Probe Value");
	...
	public class halconModelParameter
	{
        ...
        public parameterXY patternPos;          // Corner로 부터 Pattern 위치 저장
        // for projection
        public para_member quardrant;
        public para_member proj_EdgeFilter;
        public para_member proj_Direction;
        public para_member proj_Type;
        public para_member proj_MinTh;
        public para_member proj_MaxTh;
		public light_2channel_paramer light;
	}
	...
	public class pick_parameter
	{
        ...
        public para_member[] pickupVacLimit = new para_member[2];
	...
	public class place_parameter
	{
        ...
        public parameterXYZT offset2 = new parameterXYZT();
        ...
        public parameterSearch forceTracking = new parameterSearch();
		...
		public para_member[] placeForceOffset = new para_member[2];
	}
    ...
    public class parameterForce
    {
        public para_member[] forceLevel = new para_member[20];
        public para_member[] bottomForce = new para_member[20];
        public para_member[] topForce = new para_member[20];
        public para_member[] heightLevel = new para_member[20];
    }

	public class parameterHeadOffsetZ
	{
    ...
    public struct parameterXYW
    {
        public para_member x;
        public para_member y;
        public para_member w;
    }
