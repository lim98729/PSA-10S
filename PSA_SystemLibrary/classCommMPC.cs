...
namespace PSA_SystemLibrary
{
	...
	public class ClassCommMPC : CONTROL
	{
        ...
        public string inputTmsPCName = mc.para.DIAG.inputTmsPCName.description;
        public string outputTmsPCName = mc.para.DIAG.outputTmsPCName.description;
		...
		public void activate()
		{
            ...
            inputTmsPCName = mc.para.DIAG.inputTmsPCName.description;
            outputTmsPCName = mc.para.DIAG.outputTmsPCName.description;
			...
			try
			{
				...
				for (index = 0; index < mc.board.working.tmsInfo.TrayRow.I; index++)
				{
					...
					for (j = 0; j < mc.board.working.tmsInfo.TrayCol.I; j++)
					{
						...
						tmpvalue += tmpdata.ToString();		// 20140514. jhlim
					}
				...
				filename1 = "\\\\" + outputTmsPCName + machineName;
		...
		public void readTMSFile2(out bool result, int autoCtl = 0)
		{
            ...
            {
                filename = "C:\\data\\" + "TMS_PREISPT_Local.ini";
            }
            else
            {
                if ((mc.swcontrol.hwCheckSkip & 0x02) == 0)
                {
                    if (mc.para.ETC.preMachine.value == (int)PRE_MC.INSPECTION) filename = "\\\\" + inputTmsPCName + "\\TMS\\" + "Inspection_Data.txt";
                    else if (mc.para.ETC.preMachine.value == (int)PRE_MC.DISPENSER) filename = "\\\\" + inputTmsPCName + "\\tms\\" + "DP2.ini";
                    else if (mc.para.ETC.preMachine.value == (int)PRE_MC.ATTACH) filename = "\\\\" + inputTmsPCName + "\\tms\\" + "TMS_ATC.ini";
                }
			...
			if (autoCtl == 0 || autoCtl == 2)
			{
				try
				{
                    ...
                    mc.board.loading.tmsInfo.LotID = "TestLot";

                    //readNum = (int)readTmsNum.TrayID;
                    //key = "TRAYID";
                    //tmpvalue = inifile.IniReadValue(section, key, filename);
                    //mc.board.loading.tmsInfo.TrayID = tmpvalue;
                    mc.board.loading.tmsInfo.TrayID = "TestBoat";
                    ...
                    mc.log.debug.write(mc.log.CODE.INFO, "Read Map Data : " + mapData);

                    maxCount = mc.board.loading.tmsInfo.TrayCol * mc.board.loading.tmsInfo.TrayRow;
                    int curX = 0, curY = mc.board.loading.tmsInfo.TrayRow - 1;
                    tmpMap = new int[maxCount];

                    for (index = 0; index < maxCount; index++)
                    {
                        if (curX >= mc.board.loading.tmsInfo.TrayCol.I || curX < 0)
                        {
                            curY--;

                            if (mc.board.loading.tmsInfo.TrayRow % 2 == 0)
                            {
                                if (curY % 2 == 0) curX = mc.board.loading.tmsInfo.TrayCol.I - 1;
                                else curX = 0;
                            }
                            else
                            {
                                if (curY % 2 == 0) curX = 0;
                                else curX = mc.board.loading.tmsInfo.TrayCol.I - 1;
                            }
                        }
                        //else curX = index % mc.board.loading.tmsInfo.TrayCol.I;

                        tmpMap[index] = Convert.ToInt32(mapData.Substring(index, 1));
                        
                        //getPadIndex(mc.board.loading.tmsInfo.TrayCol.I, mc.board.loading.tmsInfo.TrayRow.I, curX, curY, out tmpindex, out rst);
                        if (tmpMap[index] == 0)         // good
                        {
                            mc.board.padStatus(BOARD_ZONE.LOADING, curX, curY, PAD_STATUS.READY, out ret.b);
                        }
                        else if (tmpMap[index] == 1)         // ng
                        {
                            mc.board.padStatus(BOARD_ZONE.LOADING, curX, curY, PAD_STATUS.EPOXY_POS_ERROR, out ret.b);
                        }
                        else
                        {
                            // skip
                            mc.board.padStatus(BOARD_ZONE.LOADING, curX, curY, PAD_STATUS.SKIP, out ret.b);
                        }
...
//                    }
			...
			else	// manual control
			{
				...
				for (index = 0; index < maxCount; index++)
				{
					mc.board.loading.tmsInfo.mapInfo[index] = 1;		// 1
					mc.board.loading.tmsInfo.pre_barcode[index] = "null";
					mc.board.loading.tmsInfo.post_barcode[index] = "null";
				}
			}
